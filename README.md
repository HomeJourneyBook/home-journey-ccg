...
|`ward`         |Magic-damage counterpart to Armor — full immunity to exactly the damage category that's flagged `bypassArmor=true` in `dmgCard()` (game.js): active AOE (`doUmbAsir()`/`doVardan()`), Shard (`doShardTarget()`), targeted-spell damage (`doSpellDmgTarget()`). Checked BEFORE the Armor/HP damage branch — `if(bypassArmor && hasTag(card,'ward'))` short-circuits and returns, still shakes the card for feedback. Mirror-image of Armor's scope: Armor blocks `bypassArmor=false` (regular attacks + counter-attacks + `enter_aoe`), Ward blocks `bypassArmor=true` — together they cover every `dmgCard()` call, deliberately non-overlapping. Does NOT block burn or fear (separate code paths, never call `dmgCard()`) — author call, same reasoning as Armor's burn exemption. No stacking/value — pure boolean tag, no `:N`. Renders as `ico_ward.png` via `TAG_ICONS`/`TAG_TOOLTIPS`. Live on the Солана (Solana) Mood trait, 0.66 (see Essence pricing shop) — not to be confused with the separate Solana World-trait (→`shield`, a different, one-time-any-damage mechanic; no overlap).|

**`invisible` targeting fix + Net trait change (2026-07-17):** `getTargetableCards()`
(game.js) used to read "last one left" literally as `oppField.length===1` — with several
`invisible` creatures on the field at once (e.g. 3), none of them individually qualified as
"the last one", so NONE were targetable (only the base was) — reported as a bug/exploit. Fixed:
now blocks targeting only while at least one NON-invisible ally is present; if EVERY creature
on the field is invisible, all of them become targetable (not just one at random). Separately,
the author decided the Net trait (thematically meant for future Provoke-carrying travelers like
Dreegan/Xitr, where "can't be targeted" directly contradicts "must be targeted") should no
longer grant `invisible` at all on common (non-unique) travelers — see `enter_lose`/`lose`
above. `invisible` itself is unchanged and still lives on the two unique creatures that have it
(RYVLEN, SEEKER, see Trait-slot table below) as their own one-off unique ability, not tied to
Net.
...
|`enter_lose:N`|Opponent loses N random cards from hand when this creature is played — discarded straight to the Void, no return possible. Reuses the same `lose` execution path as the future instant-spell version (see Instant table below) — see `case 'lose':` in `abilities.js`. No-op (silent) if the opponent's hand is already empty. Added 2026-07-17, **replaces `invisible` as the Net-trait tag on common (non-unique) travelers** — see "Net trait: Invisible → Lose" note under Passive `invisible` below for why. Live on TRAVELER #53 (Jeet Umbasir, `enter_lose:1`), text `"On play: Opponent loses 1 card."`. AI weight `AI_WEIGHTS.tagBonus.enter_lose = 0.66` (same value for the `lose` spell variant).|
...
|`lose:N`     |Opponent discards N random cards from hand to the Void immediately — same `lose` effect as `enter_lose` above (see `abilities.js`), just wired to `instant` timing (`card.spell`) instead of `on_enter`. Added 2026-07-17, infrastructure only — no spell card uses it yet, reserved for a future card. AI weight `AI_WEIGHTS.tagBonus.lose = 0.66`.|
...
**Update 2026-07-17**: new tags `enter_lose`/`lose` (Net trait now discards a random card
from the opponent's hand instead of granting `invisible` — see Tag System above) both scored
at `AI_WEIGHTS.tagBonus.enter_lose = AI_WEIGHTS.tagBonus.lose = 0.66` — slightly above
`draw_attack`/`necrophage`/`shield` (0.6), since it's an unconditional resource-denial effect
(no attack needed, doesn't depend on board state) but doesn't add durability/stats to the card
itself the way shield/necrophage do. Same passive-no-choice category as `armor`/`untamed`
above — no dedicated `aiTry...()` needed, `triggerAbilities()` fires it automatically on
`on_enter`/`instant` regardless of who (human or AI) played the card.
...
| `armor:N`, `burn`, `fear`, `draw_attack`, `ward`, `incarnation`, `rage`, `invisible`,
  `vanguard`, `taunt_break`, `shield`, `vampiric`, `necrophage`, `enter_draw`, `enter_lose`,
  `lose` | 0.66 (дорогой) | +1 к профильному стату |
| `provoke`, `pierce`, `bolt:N`, `heal:N` | — (эксклюзив самих Врат, не доп.тег) | входит в базовую линию |

*`enter_lose`/`lose` добавлены в таблицу 2026-07-17 (цена 0.66, автор) — тег Net теперь даёт
не `invisible`, а сброс случайной карты у противника (см. Tag System выше). Цена именно для
магазина тегов/баланса карт — вес `AI_WEIGHTS.tagBonus.enter_lose` в коде ИИ это ОТДЕЛЬНОЕ
число (тоже 0.66, но совпадение не обязательное — см. AI Module за объяснением того веса).*
...
- [ ] Посмотреть принцип работу Erase абилки, один раз вроде чинили, но походу баг.
...
- [ ] с обновлением версии до 1.02, нужно пересмотреть Правила и свериться с механиками и обновить
то что нестыкуется;
- [ ] узнать, почему даже с учетом наших файлов в предзагрузке, после одной или двух партий не закрывая страницу,
ассеты потом почему то начинают пропадать и подгружаться на ходу опять;
- [ ] глянуть, почему правила на вьетнамском частично обрезаются снизу иногда (горизонтальный режим на айпаде,
в вертикальном такого нет, другие девайсы еще не тестил);
- [ ] Окно правил для вертикальной версии айпада сделать чтоб показывались в одну страницу за раз;
- [ ] сделать слабже крт эффект в баттл логе и мулиган окне;
- [ ] В вертикальной айпад версии - в баттл логе крестик наезжает на заголовок;
- [ ] добавить фон для надписей Недостаточно эссенции и Баттлграунд фул (а то не читаемо бывает когда много карт сзади);
- [ ] добавить больше заклинаний для вариативности (брать идеи с конца этого файла) и начать балансировать Стартер,
...
придумать с новыми 1/1, артефактами и Мирами.
- [ ] после того как починили бургер кнопку (сделали по сути больше хидер), теперь хидер собственно слишком большой хах.
подумать, можно сделать скрывающийся и выезжающий хидер или что то такое.
- [ ] карты в руке игрока, в горизонтальном режиме наезжают слегка на статусбар, хотя у нас вроде есть место в зоне руки чуть выше
чтобы поднять их.
- [ ] Плегмор, возраждаясь от своего инкарнации, встает на поле и тут же поднимает за собой существо с
...
давать другой тег. например при входе в карты в игру - противник теряет карту.
  **Решено 2026-07-17** — оба пункта: (1) `getTargetableCards()` больше не блокирует таргетинг,
  когда ВСЕ существа на поле invisible — теперь можно бить любое из них (раньше "последний
  оставшийся" читался буквально как единственная карта на поле, и с 3 invisible разом целей не
  было вообще). (2) Net-трейт для рядовых травелеров теперь даёт не `invisible`, а новый тег
  `enter_lose:N` ("при входе — противник теряет N случайных карт из руки, уходят в Пустоту") —
  переключено на единственном рядовом с Net (TRAVELER #53, Jeet Umbasir). Тот же эффект `lose:N`
  заведён и под будущее заклинание (instant timing), просто пока не назначен ни одной карте. ИИ
  оценивает оба новых тега весом 0.66 (`AI_WEIGHTS.tagBonus`). `invisible` как тег никуда не делся
  из движка — остаётся уникальной способностью на RYVLEN/SEEKER, просто больше не выдаётся через
  Net на рядовых, так что конфликт с Provoke у будущих Дригана/Кситра снят на уровне дизайна.
...
- для рядовых знакончить фоны Схема, Облака, Древний и Песок;
