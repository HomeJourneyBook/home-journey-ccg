...
| Долина | ⚠ не решено — было "добор в начале хода" (технически не заведено на рядовых,
  см. ниже), сейчас автор рассматривает "при смерти дать карту" вместо этого — открытый вопрос |

**Trait-слоты без назначения (ждут решения):**
Optical Dope · Схема · Бамбук (тентативно: хил при смерти?) · Незабываемый (тентативно:
...
**Известные технические ограничения (не забыть при доработке кода):**
- Трейт "Долина → добор в начале хода" на РЯДОВОМ traveler физически не заведён —
  диспетчер `draw` в `abilities.js` даёт обычным существам только `on_attack`-тайминг,
  `on_turn` — только у world/artifact/unique. У TEANTIST работает только потому, что он
  уникальный. Нужна отдельная правка, если трейт "Долина" не поменяют на другую механику.
- Броня (`armor:N`) на РЯДОВЫХ travelers (пример: #398) пока не имеет формального места в
  правиле доп.тегов выше (правило описывает Mood/World теги, не Броню-как-доп.тег) — если
  Броня продолжит появляться на рядовых картах, стоит явно прописать её туда.
...
- [ ] Новый тег "+X ATK за каждое сыгранное заклинание" (`on_spell_cast:N`) — идея, не
  реализована, перенесена в общий бэклог механик (см. "Essence pricing shop" выше, "Свободные
  механики").
- [ ] THE BOOK (`ess_add:1`/ход) — цена приведена в порядок (cost 6, тело+способность по
  общей формуле), но сам эффект по-прежнему может ощущаться слабым/простым по сравнению с
  остальными Артефактами — пересмотр самой механики (не цены) всё ещё открыт.
