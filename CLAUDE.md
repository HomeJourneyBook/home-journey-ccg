...
_Прочитать в начале КАЖДОЙ новой сессии, прежде чем предлагать шаги._
...
механик, переехало в отдельный Roadmap (Version 1.01/2.0) сразу за этим разделом._
...
- [ ] **Подключить звуки от Муры** — файлы уже на руках у автора, принесёт завтра. См. Sound
  checklist ниже за списком конкретных мест подключения.
- [ ] **Фон модалок подбора карт и муллигана — ворота** — deckPickerModal/vsAiPickerModal/
  deckBuilderModal и mulliganScreen получают фон с воротами. Ждём арт (см. Art backlog ниже,
  пункт "Врата").
- [ ] **Анимация перехода "ворота открываются"** — при переходе от подбора карт/муллигана к
  игре: ворота начинают открываться, карты уменьшаются к центру, будто улетают внутрь. Арт для
  этого перехода ещё не нарисован — ждём (см. Art backlog ниже, пункт "Анимации").

-----

## Roadmap — версии (реорганизовано 2026-07-07)

_По прямому запросу автора: механики/баланс сознательно НЕ трогаем прямо сейчас — копятся
списком на потом (Version 1.01). Крупные технически самостоятельные блоки — ещё дальше
(Version 2.0), к ним не возвращаемся, пока не решим всё остальное._

### Version 1.01 — контент и баланс (не трогаем сейчас, только копим список)

- [ ] Unseen: cost 0 (сейчас не 0 — см. `data.js`)
...
- [ ] Протестить механику: если существо только что вошло в игру в свой ход (ещё
  `sleeping`/не ходило), не должно давать "ответку" (встречный урон себе-в-атакующего),
  если его атакуют в СЛЕДУЮЩИЙ ход соперника — сила ответного удара должна возвращаться
  только в СВОЙ ход владельца существа. Сейчас `dmgCard()`/`doAttack()` всегда наносят
  взаимный урон вне зависимости от sleeping. Уточнено автором 2026-07-08 — надо ещё раз
  свериться с автором на предмет точной формулировки перед реализацией.
...
- [ ] Апгрейд дизайна окна декбилдера — карты мелкие на лаптопе, степпер спеллов нечитаем как
  интерактив, идея «один тайл на копию спелла» вместо степпера. Частично блокировано артом
  скина окна. Возможно совпадает с "Красивая модалка подбора карт" выше — см. там.
...
код в приоритете на завтра); переход "ворота открываются" между подбором карт/муллиганом и
игрой — ворота раскрываются, карты улетают/уменьшаются к центру (арт ещё не нарисован, см.
Backlog — Code выше).

**Декбилдер (Rush) — окно выбора карт:** добавлено 2026-07-08, пока везде чёрный
плейсхолдер-фон (#0d0d1a) + кремовый текст.
- фон хидера (сейчас плоский цвет #5a7278)
- фон-плашка под блок статистики (Selected/Travelers/Uniques/Spells/Worlds/Artifacts)
- фон-плашка под диаграмму кривой маны
- кнопки-фильтры сортировки — арт уже частично подключен (`sort_all/traveler/unique/
  spell/world/artifact` × `1/H/2`, автор прислал 2026-07-08), но у самих кнопок пока нет
  своего фона под ними — нужен единый "персональный" фон-плашка под весь ряд кнопок
  (сейчас сами кнопки просто плавают на фоне хидера)

**Модалки подбора карт/муллиган:** фон с воротами для deckPickerModal/vsAiPickerModal/
deckBuilderModal и mulliganScreen (см. Backlog — Code выше) — ждём арт. Также нужен
отдельный арт для НЕАКТИВНОГО (disabled) состояния кнопки Mulligan (`btn_mulligan`
когда мулиганить больше нельзя) — сейчас это просто дефолтная арт-версия с
opacity:0.4, нужен кастомный "выключенный" вид.

**Персонажи:** концепт Шен (маскот) — добавлено 2026-07-07. Идея с самим маскотом и
всплывающими подсказками пока в Ideas parking lot ниже, отдельно от концепт-арта.
...
## Sound checklist — от Муры, файлы на руках (записано 2026-07-06, обновлено 2026-07-08)

_Файлы уже у автора. Большая часть подключена в этой сессии — см. пометки. Заодно
обновлены на более свежие версии от Муры: `card_navigation_cursor` (вместо старого
`Navigation_Cursor`), `card_burn` (вместо `Burn_Card`), `yellow_buttom` (вместо
`yellow_buttom_play_endturn_menu_gravyard_loop`), `card_select_traveler` (вместо
`Click_Cursor` на превью карты в руке), и фоновая музыка `main_theme.mp3` (вместо
`Main_theme.mp3` — Мура что-то подкрутил в свежей версии)._
...
- [ ] Баф (сделать громче) — файл `baf.wav` не менялся, громкость не трогал — отдельная
  правка `SFX_VOLUME`/индивидуальной громкости, скажи если нужно сделать сейчас
- [ ] Дебаф (погромче) — аналогично, громкость не трогал
- [ ] Звук «тык» (чуть-чуть громче) — не трогал громкость
...
- [ ] Победа — фанфары при появлении win-модалки — ждём файл от Муры
...
## Ideas parking lot — 2026-07-06 (почищено 2026-07-07)
...
приоритетов нет; при созревании идея переезжает в Backlog — Code или Roadmap с приоритетом._
...
- Маскот (Шен) со всплывающими подсказками (концепт-арт уже в Art backlog выше).
