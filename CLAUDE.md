...
- [ ] **Добор карт вне начала хода — без анимации/звука (Hunger/Altar/spell draw "draw N
  ...
  откачены (см. журнал сессии выше, "Откат попыток фикса анимации добора"). Реальная причина
  всё ещё не найдена — оба захода были основаны на гипотезах о том, где именно `rZone()`
  (render.js) теряет анимацию для этих конкретных источников, ни одна не подтвердилась.
  Единственное, что реально помогло в процессе — не связанный с этим фикс: скрытая рука
  соперника в hotseat больше не переигрывает анимацию/звук при каждой передаче хода
  (`rHiddenHand()` — анимация/звук убраны оттуда полностью, это осталось в силе).
  Следующий заход стоит начать не со статического чтения кода, а с логирования прямо в
  браузере автора в момент срабатывания одного из этих источников (например
  `console.log`/`debugger` внутри `rZone()` на условии `isNew`) — статический разбор кода
  дважды не попал в реальную причину.
