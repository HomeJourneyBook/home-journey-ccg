...
### Базовые кривые hp/atk для рядовых архетипов (актуальная версия, ребаланс 2026-07-20)

> **Оговорка об источнике истины (2026-07-21):** утренний ребаланс 2026-07-21 точечно
> сдвинул статы части карт относительно "живых чисел" в примерах ниже (например, Xuiqtr
> cost3 в data.js живёт как 2/5, а не 2/6 из таблицы). Правило разрешения конфликтов:
> **формулы ниже — ориентир для НОВЫХ карт, фактические статы соседей по архетипу/costу
> в data.js — истина при расхождении.** Плюс обязательный шаг 4 чеклиста ниже (симулятор) —
> он ловит то, что формула не поймала.
...
по отдельности. **Заменяет устаревшее правило "Orbiton/Umbasir — правило симметрии с Merchird"
(закреплено 2026-07-18, см. дальше по файлу) — та таблица больше не действует, ниже — актуальная.**

Общий принцип компенсации: если у новой карты архетипа есть "премиальный" тег сверх
сигнатурного для архетипа (armor/vanguard/shield/enter_heal/enter_draw/fear/taunt_break и
т.п.), можно свободно снять 1 hp или 1 atk от целевого значения по формуле — это МЯГКИЙ
ориентир (посмотри, как уже отличаются варианты одного архетипа/costа между собой в
`data.js`), а не жёсткая математика вроде "+1 hp за тег" (та математика была в старом правиле
2026-07-18 и от неё отказались — слишком легко уводит карты далеко от эталона архетипа, если
тегов на карте несколько).

**Szarg — опорный архетип (gtype:szg), "чистый" атакующий:**
```
HP  = cost
ATK = cost + 1
```
Живые числа: cost1 → 1/2, cost2 → 1-2/2, cost3 → 2-3/3, cost4 → 3/4.

**Orbiton — хилер (gtype:orb), сигнатурный тег `heal:2` + Active Heal/Squad Heal:**
```
HP  = cost   (как у Szarg)
ATK = 1      (фиксированно на любом cost)
```

**Umbasir — болтер/утилити (gtype:umb), сигнатурный тег `bolt:1` + Active Bolt/Squad Bolt:**
```
HP  = cost   (как у Szarg и Orbiton — раньше было cost−1, выровнено 2026-07-20)
ATK = 1      (фиксированно)
```

**Mechird — пробивной атакующий (gtype:mch), сигнатурный тег `pierce`:**
```
HP  = cost
ATK = max(1, cost − 2)   (заметно ниже Szarg на этом же cost — компенсация через pierce,
                          эффективно бьёт больнее по бронированным целям)
```
Живые числа: cost2-3 → atk1, cost4 → atk2, cost5 → atk3.

**Dreegan — танк (gtype:drg), сигнатурный тег `provoke` — СВОЯ кривая, сознательно НЕ
трогали 2026-07-20 (автор подтвердил, что она в порядке):**
```
HP  ≈ 2 × cost   (начиная с cost3; cost2 держится ниже, ~cost, как исключение для дешёвого дропа)
ATK = 1          (фиксированно)
```
Живые числа: cost2 → 2, cost3 → 6-7, cost4 → 8-10, cost6 → 13. Это эталон "hp вместо atk"
для Xuiqtr ниже.

**Xuiqtr — гибрид (gtype:xui), сигнатурный тег `intercept` — двухфазная кривая:**
```
cost 1-3:  HP как у Dreegan на этом же cost (см. выше); ATK = cost − 1 (кроме cost2 → 1,
           исключение для дешёвого дропа, как и у Dreegan/Szarg на cost1-2)
cost 4+:   HP = cost + 2       (рост резко замедляется — НЕ продолжает ×2 как у Dreegan)
           ATK = cost − 1      (рост ATK продолжается тем же темпом, что и на cost3)
```
Живые числа: cost2 → 2/1, cost3 → 6/2, cost4 → 6/3, cost5 → 7/4, cost6 → 8/5.
...
### Гармония архетипов — проверенный базис v1.04 (2026-07-21, симулятор, 2×1000 партий)

Зафиксированное состояние, относительно которого меряются все будущие правки. Два
независимых прогона 1000 партий (чередование первого игрока): фракции **49.5/50.5** и
**48.1/51.9** — паритет в коридоре. Медиана партии 13 ходов (диапазон 7-23), фатиг <1%.

**Агрегированный winrate-when-played по архетипам** (все карты архетипа, 2-3 тыс.
розыгрышей на архетип): xui 55.5 / drg 52.9 / szg 52.4 / mch 51.5 / umb 50.5 / orb 47.3.
Целевой коридор архетипа: **46-56**. Сейчас все внутри; края (xui сверху, orb снизу) —
первые кандидаты на присмотр, но НЕ на правку без выхода за коридор на 2+ независимых
прогонах.

**Доли урона по базам среди рядовых** (уники исключены): szg 28% (эталонный атакер — верхняя
доля уместна), drg/mch по 18%, umb 17%, xui 16%, orb 4% (хилер — низ ожидаем). Историческая
доминация mch (45-70% в логах до pierce-редизайна и разъезда архетипов) закрыта: pierce-фракция
теперь всегда играет ПРОТИВ provoke-фракции — встроенная камень-ножницы стартеров.

**Известные особенности базиса (наблюдать, не чинить вслепую):**
- Уники наносят **51% всего урона по базам** — партии добивают легендарки. Это соответствует
  замыслу "уник = праздник", но если живые игроки почувствуют "рядовые не решают" — рычаг
  известен (срезать HP топ-эндов или поднять цену), симулятор проверит за минуту.
- Винрейт ПЕРВОГО игрока 58-61% при цели ~52-53 (эталон HS с Монеткой). Отдельный workstream:
  сперва проверить, не в ИИ ли дело (консервативная `ess_add`-ветка недоигрывает бонус P2),
  только потом трогать саму компенсацию. НЕ смешивать с правками баланса фракций — по одной
  переменной за раз.
...
- [ ] отредактировать Правила под версию 1.03

-----

### Итог сессии 2026-07-21 (вечер) — стартеры "3 Врат против 3 Врат", дека 35, замена бонуса P2, headless-симулятор

Версия: `GAME_VERSION` 1.03 → **1.04** (состав дек и бонус второго игрока изменены — старые
deck-JSON и battle-логи будут помечаться как устаревшие, это ожидаемо).

**1. Бонус второго игрока: UNSEEN → фракционный 0-cost спелл** (`grantUnseenBonus()` в
ui.js). Второй игрок теперь получает SCHEME (Tea) / BLACK MAGIC (Jeet) — "+1 эссенция на
этот ход", прямой аналог Монетки Hearthstone. Обоснование — в аудите (HJ_CCG_Balance_Audit):
0-cost масс-баунс был самой свинговой картой игры и наказывал построение поля. Сами карты
уже были в DEFS с `SPELL_COPIES=0` — в роли разовой компенсации это ровно их вес; ИИ играет
их через существующую `ess_add`-ветку `aiScoreCard()` (разыгрывает, только когда прибавка
что-то открывает — правок ИИ не потребовалось). UNSEEN остался в DEFS для каталога, цена
поднята 0→6 как у кандидата в будущий нейтральный пул.

**2. Classic пересобран: "3 Врат против 3 Врат", 35 карт/фракцию.** Tea = Szarg 6 /
Orbiton 5 / Dreegan 5; Jeet = Umbasir 6 / Mechird 5 / Xuiqtr 5 (квоты чужих архетипов = 0,
их массивы в `_composeDeckList()` целиком в резерве — карты живы в DEFS/каталоге). 16 рядовых
+ 5 уников + 12 спеллов + мир + артефакт = 35. Мотивация (полный разбор — в аудите): при
старых квотах 8 из 12 фракционных архетипов физически не могли собрать Squad-тройку (мёртвый
текст на картах); тел было 47.5% при жанровой норме ~60%; при деке 40 и медианной партии
~14 ходов игрок видел меньше половины колоды. Спелл-резка асимметрична и матчап-осознана:
Tea потеряла EXPOSE (у Jeet провока на рядовых больше нет — только ABYSSWALKER), Jeet
сохранил UNMASK (единственный взлом Dreegan-стены Tea) и SWARM CULL (Tea теперь wide-фракция).

**3. Headless-симулятор self-play** (`sim/headless.js`) + A/B-тюнер весов ИИ
(`sim/tune.js`). Грузит настоящие движковые файлы в vm-контекст с 7 UI-заглушками (список
получен статическим анализом), партия AI-vs-AI играется за ~50 мс. Метрики за прогон:
винрейт фракций, винрейт первого игрока, длины партий, доля фатиг-концовок, урон по базам
в разрезе gtype (та метрика, что считалась руками по логам), и per-card
**winrate-when-played** (метрика HSReplay). Это ЗАМЕНЯЕТ ручной цикл "battle_log → разбор
в сессии" для количественных вопросов; ручные логи остаются нужны для качественных
("выглядит ли ход глупо").

**4. Первые данные симулятора** (1000 партий, чередование первого игрока):
- v1.04 как собрано: Tea 42.8% / Jeet 57.2% — вне коридора 45-55. Топ по
  winrate-when-played — весь Jeet: RYVLEN 69.9%(!), ABYSSWALKER 66%, FRENZY 63.7%,
  Xuiqtr-пакет #720/#951/#579 60-64%. Низ — весь Tea: орбитон-хилеры #218/#398/#433/#10
  39-43%, WILDFIRE 42.9%.
- Правка по данным: Tea Szarg #890 (ванильный однодроп, 39.9% WWP — худшее тело Tea) →
  резерв, вместо него #34 (cost3 4/3 regen). Повторный прогон 1000: **Tea 45.2% /
  Jeet 54.8%** — кромка коридора. Дальше полировать хочется правками Jeet-топа (кандидаты:
  RYVLEN cost 5→6 или без enter_draw; FRENZY/CARAPACE cost +1), но это уже правки статов —
  решение за автором, симулятор проверит за минуту.
- Винрейт первого игрока 57.8-58.2% — выше идеала (~52-53% у HS с Монеткой). Возможные
  причины: ИИ недоигрывает бонусную карту (ess_add-ветка консервативна) либо компенсация
  мала. Наблюдать; НЕ чинить одновременно с балансом фракций — две переменные разом.
- Оговорка к per-card метрике: WWP карты-бонуса P2 (SCHEME 33.5%) — артефакт корреляции
  "играешь её ⇔ ты второй игрок", не слабость карты. При интерпретации таблицы это
  касается ЛЮБОЙ карты, коррелирующей с состоянием (revive-спеллы коррелируют с "мои
  существа умирали" и т.п.) — WWP это сигнал для расследования, не приговор.

**5. Нерфы по данным симулятора (тот же вечер, по прямому запросу автора):**
RYVLEN cost 5→6 (был 69.9% winrate-when-played — максимум по всей игре; статы/теги не
тронуты, HP сознательно НЕ поднят до кривой cost6); ARCHIVE/FRENZY: +2 ATK → +1 ATK,
цена 3→2 (оба зеркала вместе — FRENZY был 63.7% WWP). Контрольный прогон 1000 партий:
**Tea 48.1% / Jeet 51.9%** — уверенно в коридоре 45-55, почти паритет. RYVLEN после нерфа
63.6% WWP — всё ещё топ, но рядом с ним теперь TEANTIST 63.3% (draw-уники сильны как
класс); держать в уме, что у поздних уников WWP частично завышен селекцией — "разыграл
дорогую карту" коррелирует с "дожил до late game не проигрывая", это не чистый сигнал.

**Открытое после сессии:** (а) решить по нерфам Jeet-топа и прогнать; (б) tune.js прогнать
на 300+ партиях на кандидата (на 120 всё в шуме, ожидаемо); (в) Rush-пул теперь следует за
Classic и содержит только 3 архетипа на фракцию — если для Rush хочется все 6, нужен
отдельный флаг пула (сознательно НЕ делалось в этой сессии); (г) муллиган в симуляторе —
политика "keep any hand", можно добавить эвристику для реалистичности.

### Итог правок 2026-07-20 — иконки без двоеточия, Bolt 2 спеллы, ребаланс 4 архетипов

- [x] Убрано двоеточие после "On play"/"On turn"/"On attack"/"Active"/"Squad" во всех текстах
  карт (`ab` в `data.js`) — `formatAbilityText()` переписан так, чтобы матчить слово-триггер
  независимо от наличия двоеточия и не дописывать его обратно после иконки (раньше иконка
  формально требовала двоеточие в исходном тексте).
- [x] Новый спелл Bolt 2 (cost 2) на каждую фракцию — Tea: **SPARK** (⚡, `t_sp15`), Jeet:
  **MALICE** (⚔️, `j_sp15`), оба `spell_dmg_target:2`. По 1 копии в Classic-старте (`SPELL_COPIES`
  в `deck.js`), взамен срезанной 1 копии Bolt 3 (JOURNEY `t_sp2` 2→1, HEX `j_sp13` 2→1) —
  итоговый размер колоды не изменился (46/фракцию). img намеренно не указан у обеих карт —
  арт ещё не готов, рендерятся через emoji-фолбэк (`card.art`), см. `render.js`.
- [x] Ребаланс статов Orbiton/Umbasir/Mechird/Xuiqtr — см. новую живую формулу-таблицу в
  "Adding a New Card" (раздел "Базовые кривые hp/atk для рядовых архетипов", заменяет
  устаревшее правило 2026-07-18 "Orbiton/Umbasir — правило симметрии с Merchird" ниже по файлу).
  Коротко: Orbiton/Umbasir HP срезан до hp=cost (как у Szarg, раньше был выше/по формуле
  "+1 hp за доп.тег"); Mechird ATK срезан до max(1,cost-2) при hp=cost; Xuiqtr HP поднят до
  уровня Dreegan на cost1-3, но с cost4+ HP растёт медленно (+1/cost) а ATK — быстро (cost-1).
  Szarg/Dreegan не трогали — Dreegan подтверждён автором как эталон.
- [x] `GAME_VERSION` НЕ поднимали — не было явного запроса (см. дисциплину в Session Workflow
  выше).
...
### Starter Set 2 — «Отражённые Врата» (план, 2026-07-21, НЕ реализовано)

Прямое следствие разъезда архетипов: вторые три Врат каждой фракции уже лежат резервами в
хвостах массивов `_composeDeckList()`. Starter Set 2 = **Tea играет Umbasir/Mechird/Xuiqtr,
Jeet — Szarg/Orbiton/Dreegan** — зеркало первого стартера без единой новой механики.

**Что это даёт:** четыре осмысленных матчапа вместо одного (Set1 vs Set1, Set2 vs Set2 и два
кросса Set1 vs Set2), второй физический продукт для печати, и полный охват коллекции —
каждый архетип каждой фракции оказывается хоть в одном стартере.

**Что нужно сделать (по порядку):**
1. Инвентаризация резервов: Tea umb 6 / mch 6 / xui 4 карт в DEFS, Jeet szg 6 / orb 4 /
   drg 5 — Tea-xui не хватает 1-2 карт до квоты 5-6, Jeet-orb на нижней границе. Добрать
   из НФТ-коллекции по кривым архетипов (чеклист выше) — это ЕДИНСТВЕННЫЕ новые карты.
2. `DECK_CONFIGS.classic2` со своими `archetypeSizes` (зеркальные квоты) — `_composeDeckList()`
   уже параметризован конфигом, правка минимальна; в UI — второй пресет в "Choose Your Deck".
3. Спеллы: стартовать с ТЕХ ЖЕ 12 спеллов на фракцию, но пере-решить матчап-тех зеркально:
   в Set 2 провок-стена у ДЖИТА (drg) — значит EXPOSE возвращается Tea, а UNMASK у Jeet
   теряет цель; SWARM CULL/RECKONING переоценить по тому, кто теперь wide-сторона.
4. Темы Burn/Fear проверить на плотность в новых составах (burn-карты Tea-umb/mch/xui:
   #921/#387/#972 — есть; fear-карты Jeet-szg/orb/drg: #434/#523/#859 — есть; но пересчитать
   точно при сборке).
5. Симулятор: все 4 матчапа по 1000 партий, коридор 45-55 в КАЖДОМ. Для кроссов понадобится
   микро-доработка headless.js: параметр "какой конфиг деки у какой фракции" (сейчас обе
   стороны берут один deckConfig — единственное техническое препятствие, ~20 строк).
6. Только после коридора — печать. Set 2 сознательно НЕ раньше, чем Set 1 обкатан живыми
   партиями: зеркало наследует все правки баланса Set 1 бесплатно, спешить некуда.

### Version 2.0 — большие блоки (детализировано 2026-07-21, порядок = рекомендуемая очередность)

**Предусловие для ВСЕГО блока:** Set 1 показан сообществу и обкатан живыми партиями,
винрейты в коридоре и по симулятору, и по живым логам. v2.0 строится на стабильном ядре.

- [ ] **Шаг 0 — Онлайн-фундамент: авторитетное состояние.** Сейчас G живёт в браузере и оба
  игрока ему доверяют (hotseat). Для онлайна нужен сервер-арбитр. Хорошая новость: движок
  уже почти готов к этому — headless-симулятор доказал, что game.js исполняется в чистом
  node без DOM (7 заглушек). План: (а) вынести чистое ядро (data/deck/state/abilities/game)
  в общий модуль клиент+сервер; (б) сервер держит G, клиенты шлют интенты ("play card id",
  "attack A→B"), сервер валидирует ТЕМИ ЖЕ функциями (doPlay/doAttack) и рассылает дельты;
  (в) скрытая информация (рука соперника, колода) — сервер отдаёт каждому клиенту только
  его срез G, рендер уже умеет "закрытую" руку из hotseat/vsAI. Транспорт — WebSocket,
  первый милстоун: комната на двоих по ссылке, без матчмейкинга/рейтинга.
- [ ] **Шаг 1 — Web3 / NFT-интеграция.** Полный архитектурный план УЖЕ зафиксирован — раздел
  "Deckbuilding Rules — Marketplace Era" выше по файлу (индексер → маппинг NFT→ключ DEFS
  через attributes.card_key → `{key,qty}` в формате getRushPool() → флаг enabled). Порядок
  внедрения: сперва read-only "подключи кошелёк → увидь свою коллекцию в каталоге"
  (ноль игровых последствий, чистая витрина), потом constructed-режим по лимитам из того же
  раздела (≤3 копии рядового/спелла, ≤1 уник, ≤5 на gtype). Открытые решения оттуда же не
  забыть: судьба нейтралок и лимиты для buildAiRushDeck().
- [ ] **Шаг 2 — Тренировочная игра (демо).** Заскриптованная обучающая партия. Техплан:
  сценарий = массив шагов {фиксированная рука/колода обеих сторон, ожидаемое действие
  игрока, реплика Шена}; движок поверх game.js с seed-ованными деками (initState уже
  принимает rushDecks — фиксированная колода бесплатна) + блокировка всех действий кроме
  ожидаемого. Контент: 3 сценария — "первое существо и атака", "провок/перехват и размен",
  "сквад и активки". Естественная точка для дебюта маскота Шена (см. ниже) — обучение и
  подсказки один контент-пайплайн.
- [ ] **Шен (маскот)** — всплывающие подсказки в обычных партиях (первая партия игрока:
  подсказки на первые N ходов, отключаемые) + голос обучалки. Арт-зависимость: спрайты Шена.
- [ ] **Расширение состава существ на поле** (кроме Путешественников и 1/1) — БЛОКИРОВАНО
  ...
  Не трогать до закрытия Шагов 0-1.
...
- [ ] звук снятия Solana Shield (момент когда это делает абсорб)
- [ ] звук когда играешь спелл дать эссенцию
