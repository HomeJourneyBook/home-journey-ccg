...
- **Gates** (the body) — maps to the archetype/gtype grouping. A future task: show each
  ...
  **Author calls travelers by their Gate name in conversation** (e.g. “Orbiton”, “Merchird”) —
  ...
  |Xuiqtr   |`xui`       |`provoke` (+fear/burn/untamed)  |Squad: Regen 2.                     |
  
  2026-07-11: пересмотрены роли Врат — теперь чёткая структура "2 танка / 2 атакующих /
  2 заклинателя": Dreegan+Xuiqtr оба несут `provoke` (два разных танка — один со Squad-бронёй,
  другой со Squad-регеном), Szarg (`vanguard`, быстрый агрессор) и Merchird (`pierce`,
  пробивной агрессор) — два "атакующих" архетипа, Orbiton/Umbasir — два "заклинателя"
  (хил/аое). Xuiqtr раньше нёс `regen:1` как базовый тег — теперь регенерация ТОЛЬКО
  от Squad-бонуса (Regen 2 при 3+ на поле), у отдельной карты вне отряда её нет вообще.
  Все 8 существующих Xuiqtr-травелеров (`t_trvl187_w`, `t_trvl704_w`, `t_trvl26_w`,
  `t_trvl39_w`, `j_trvl579_w`, `j_trvl972_w`, `j_trvl50_w`, `j_trvl37_w`) обновлены
  в `data.js` — старые доп. флейвор-теги (fear/burn/untamed) сохранены как были.
...
### Essence pricing shop (v1.01) — формула цены рядовых путешественников

Зафиксировано 2026-07-11, применено ко всем 48 не-уникальным travelers в `data.js` в этой
версии. Идея: вместо подгонки cost/hp/atk на глаз — единая формула-"магазин", где каждое
очко статов и каждый тег имеет явную цену в эссенции. Позволяет пересчитать баланс заново
в любой момент, если решим подвинуть какую-то цифру.

**Цена "тела" (без тегов):**
- Налог за само тело: **0.2 эссенции** (платится всегда, просто за факт существования карты)
- HP: **0.2 эссенции/очко**
- ATK: **0.4 эссенции/очко** (вдвое дороже HP)
- Проверка на якоре: 1 HP + 1 ATK = 0.2+0.2+0.4 = **1.0 эссенции** ровно.

**Цена тегов (используются на рядовых travelers):**

| Тег | Цена |
|---|---|
| `gtype:xxx` (сама категория архетипа) | 0 |
| `provoke` | 1.5 |
| `rage` (даже с потолком накопления) | 1.5 |
| `aoe:1` (активка) | 1.25 |
| `vanguard` | 1.0 |
| `pierce` | 1.0 |
| `heal:1` (активка) | 1.0 |
| `burn` | 1.0 |
| `incarnation:4` (фикс. для рядовых — см. ниже) | 1.0 |
| `fear` | 0.75 |
| `untamed` | 0.75 |
| `regen` | 0.75 |

**Базовый тег и цена-старт по вратам** (голый гейт-тег без доп.трейтов):

| Gate | Базовый тег | Cost-старт | Перекос статов |
|---|---|---|---|
| Szarg | `vanguard` | 2 | ATK (HP-пол растёт с ценой — 1 на cost2-3, **2 на cost4-5**: дешёвую глазную пушку терять не жалко, дорогую — уже жалко) |
| Merchird | `pierce` | 2 | ATK (тот же растущий HP-пол: 1 на cost2-3, 2 на cost4-5) |
| Dreegan | `provoke` | 3 | HP (потолок **HP 5** — выше не раздуваем, излишек уходит в ATK) |
| Xuiqtr | `provoke` | 3 | HP (тот же потолок HP 5) |
| Orbiton | `heal:1` | 2 | Минимум по обеим (дёшево, сила — в кнопке), тоже потолок HP 5 |
| Umbasir | `aoe:1` | 3 | Минимум по обеим, тоже потолок HP 5 |

**Правило доп.трейтов**: каждый доп.тег сверх базового гейт-тега — **+1 к итоговой цене карты**
(не буквально доплата по прайсу тега, а фиксированный шаг цены за штуку тега; сам прайс тега
из таблицы выше уходит "внутрь" бюджета статов — чем дороже тег, тем меньше остаётся на HP/ATK
при том же шаге цены). Остаток бюджета конвертируется в HP/ATK согласно перекосу гейта;
дробный излишек — округляется в пользу того стата, что и так в приоритете у этого архетипа.

**Инкарнация у рядовых — зафиксировано**: всегда `incarnation:4` (не варьируется по карте),
цена 1.0 эссенции. У уникальных 1/1 значение X остаётся свободным (см. PHLEGMOR `incarnation:3`)
— вариативность параметра приберегаем для уников, у массовых карт — единое число для простоты.

**Известные результаты применения** (для справки, не переписывать вручную при доработке кода):
самый резкий сдвиг цены — TRAVELER #25 (szg, vanguard+untamed): было cost1, стало cost3 —
раньше `untamed` не был оценён вообще (цена 0), теперь честно куплен за 0.75.
