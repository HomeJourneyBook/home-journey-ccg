...
----
### Итог сессии 2026-07-27 — сверка NFT-метадаты, Mood/World-трейты, новые карты

- Полная сверка живых `data.js`-тегов с полной метадатой NFT-коллекции (`full_mint.zip`,
  1113 файлов) — построчный кросс-табличный аудит подтвердил чистое соответствие Mood/World
  → тег почти везде (см. новую сводную таблицу в разделе "Traveler Structure (NFT)" выше).
  Найдены и исправлены 2 реальные нестыковки (см. ниже); 2 других (#523/#867) оказались
  ложной тревогой — это будущие Орбитоны, ждут ручного ввода 30 карт архетипа.
- **Исправлено:**
  - `t_trvl1068_w` → `t_trvl1066_w` ("TRAVELER #1066") — опечатка в номере NFT.
  - TRAVELER #295: тег `vampiric` → `rage` (несёт World Unforgotten, не Blood).
  - Gate-таблица (Lore & Universe context): Xuiqtr исправлен на `intercept` (устаревшая
    запись всё ещё говорила `provoke`, хотя сам тег сменился ещё 2026-07-17).
- **Трейт-перестановка (по прямому запросу автора):**
  - `death_bolt:N` переехал с трейта **Scheme** на **Pink Clouds** — новое имя
    **"Thunder Storm"**, новая иконка `ico_cloud.png`. Эффект (магический урон случайному
    врагу при смерти носителя) не менялся.
  - Scheme получил взамен НОВЫЙ тег **`death_armor:N`** (иконка осталась прежней,
    `ico_scheme.png`) — при смерти носителя даёт N Брони случайному союзнику (не
    обязательно раненому). Реализовано в `killCard()` (game.js) через одноразовый
    `spellArmorBonus`+`recalcArmor()`, тот же паттерн, что у BULWARK/CARAPACE. 4 карты,
    ранее носившие `death_bolt:4` под именем Scheme (#56, #699, #26, #550), переведены на
    `death_armor:2`.
  - `vanguard` переименован в отображении в **Swiftness / Стремительность** (тултип,
    rules-book на всех 4 языках) — сам тег и иконка `ico_vanguard.png` не менялись.
- **Названы (без кода) 4 ультраредких Mood-трейта:** Orange from FFF → Foxy Skill (50%
  уклонение от урона), Saga from Krtv → Saga (синергия с парой 1/1, тег позже), To the Moon
  with DHD → Game of Market (50/50 +2 урона/−2 HP себе при атаке), Winter from RGB → Frost
  Attack (заморозка при атаке, механика позже).
- **Новые travelers:**
  - TRAVELER #250 (Tea, Szarg, cost5): `burn`+`death_bolt:4` → 5/6 по формуле Szarg.
  - TRAVELER #607 (Tea, Mechird, cost5): `draw_attack:1`+`remember` → 5/4 по формуле Mechird.
  - TRAVELER #481 (Jeet, Dreegan, cost4): `untamed`+`death_bolt:4` → 8/2 по формуле Dreegan.
  - Все три добавлены в `DEFS` (data.js) — автоматически видны в Rush/деккбилдере через
    `getRushPool()` (читает DEFS напрямую); в фиксированные Classic-колоды
    (`CLASSIC_TEA_DECK`/`CLASSIC_JEET_DECK`) НЕ добавлялись, не просили.
  - ⚠️ TRAVELER #250 (cost5 Szarg с двумя дорогими доп.тегами) — см. отдельную заметку
    "cost5 Szarg — решение отложено" выше: формула без урезки ранее считалась потенциально
    перегретой по симуляции. Статы посчитаны строго по формуле как есть, не откорректированы
    превентивно — на усмотрение автора после плейтеста.
- **Пересчитаны по формуле Szarg (HP=cost, ATK=cost+1) после смены cost 4→3:**
  TRAVELER #56 (4/5→3/4), TRAVELER #734 (4/5→3/4).

----
### NFT-реестр (nft-data/nft-registry.js) — заведён 2026-07-27

Новый служебный файл `nft-data/nft-registry.js` — НЕ подключён к игре, чисто справочный склад
метадаты коллекции, чтобы автору не пересылать архивы заново каждую сессию. Содержит:
1. `TRAVELER_METADATA_AT_MINT` — метадата (World/Gate/Mood) НА МОМЕНТ МИНТА для 1110 обычных
   travelers (из `full_mint.zip`).
2. `ONE_OF_ONES_KNOWN` — 3 уникальных 1/1 без Gate/Mood/World (Aslex #1111, Tuborg #1112,
   Fireon #1110). Остальные 1/1 из коллекции автор опишет позже.
3. `ORBITON_CONVERSIONS` — 30 travelers, переводимых в Orbiton (из `NFTS.txt`), с трейтами на
   момент минта и заметкой автора, что меняется при конверсии.
4. `CURRENT_SUPPLY_SNAPSHOT` — 801 адрес текущего supply с Tensor (collection mint
   `9L6y6915VoA2uEaF7qdPkWA9FgLzLfvFCyZau32PqQpz`). Официальный supply на Tensor — 809,
   расхождение ~8 не объяснено.
5. `UNCLASSIFIED_ADDRESS_LIST_36` — 36 адресов из файла, ошибочно названного
   "11travelers.json" — назначение пока не объяснено автором.

**Находка этой сессии:** сверка 30 адресов из `ORBITON_CONVERSIONS` с `CURRENT_SUPPLY_SNAPSHOT`
показала, что 5 из них (#433, #409, #523, #596, #454) отсутствуют в текущем supply-снепшоте —
гипотеза: эти 5 конвертированы в Orbiton через burn+remint (новый адрес), а остальные 25 —
через update-in-place метадаты (тот же адрес сохранился). НЕ подтверждено историей транзакций
on-chain, только по одному снепшоту — проверить отдельно, прежде чем полагаться на это как факт.

**Открытые вопросы (ждут автора):** остальные 1080 travelers из `TRAVELER_METADATA_AT_MINT` не
привязаны к mint-адресам вообще — без этой связки нельзя проверить, кто из НЕ-Orbiton travelers
сожжён/заменён на 1/1. Природа `UNCLASSIFIED_ADDRESS_LIST_36` неясна. Полный список всех 1/1 из
коллекции ещё не собран.

----
### list-travelers.js — сводные таблицы 1/1 и рядовых (2026-07-27)

Новый файл `nft-data/list-travelers.js` (не подключён к игре) — две markdown-таблицы в
комментарии: (1) все 10 уников (1/1) сейчас в игре, (2) все 109 рядовых travelers в data.js.
Столбцы: mint address / имя в игре / cost/hp/atk / tags / текст способности / заметки.

**Находки:**
- Файлы арта уников пронумерованы 001-011, но карты с номером 006 в игре НЕТ — похоже,
  существует 11-й 1/1 персонаж, ещё не введённый. Имя `11travelers.json` (36 адресов) теперь
  читается буквально — вероятно, это 11 уникальных 1/1 персонажей коллекции.
- 3 адреса из тех 36 — это Aslex/Tuborg/Fireon (подтверждено автором), но БЕЗ доступа к
  on-chain метадате по каждому адресу нельзя определить, какой адрес какому имени
  соответствует — вписано как "не определён" у всех 10 уников, ждём данные от автора.
- Коллекционный 1/1 "Fireon" vs игровая карта "FAERON" — разное написание, не подтверждено,
  что это один и тот же персонаж — уточнить у автора.
- Автоматическая сверка тегов рядовых travelers против Mood/World-метадаты (там, где нет
  зафиксированного changeNote) дала только 2 реальных флага: TRAVELER #0 (номер не найден в
  метадате коллекции вообще) и TRAVELER #867 (Gate в метадате Szarg Mono, не входит в список
  30 известных Orbiton-конверсий — отдельный случай, требует проверки). Остальные ~18
  Orbiton-карт и их changeNote-пометки помечены информационно, не как проблема.

----
### Новый тег `frost` ("Frost Attack") — первый ультралегендарный Mood-трейт, 2026-07-27

Winter from RGB (ультраредкий Mood, 5 шт. в коллекции) получил код: `frost`, on_attack эффект,
иконка `ico_snow.png` (тег), оверлей на поле `frost.png` (110% размера карты, сверху).

**Механика Заморозки (`card.frozen`+`card.frozenTurnsLeft`):**
- На атаке вешает на цель 2 СВОИХ хода владельца (не 1, как Fear) — тикает в `endTurn()`
  тем же событием, что снимает Fear (конец хода ВЛАДЕЛЬЦА цели), но снимается только когда
  счётчик дойдёт до 0, т.е. переживает два подряд собственных хода владельца.
- Пока заморожена: карта не может действовать ВООБЩЕ — ни атаковать, ни использовать Active
  (кнопка Bolt/Heal). Добавлено `!c.frozen`/`||att.frozen` во ВСЕ точки кода, что раньше
  проверяли `feared` для этой цели (click-хендлер выбора карты, canAttackBase-подобные
  проверки, doVardan()/doUmbBolt(), + аналогичные фильтры в ai.js — свыше 20 мест).
- НЕ форсит Provoke/Intercept, пока заморожена — тот же принцип, что уже даёт `!c.feared`
  везде (4 фильтра в game.js + зеркальные в ai.js).
- Замороженная карта НЕ контратакует (`wasFrozenBefore` в `doAttack()`, тот же паттерн
  "снимок до эффектов этого удара", что у `wasFearedBefore`/`wasExhaustedBefore` — иначе
  Frost-атакующий бил бы первый раз безнаказанно).
- Заморозка СНИМАЕТ Fear/Burn с цели при наложении (простой оверрайт), и пока карта
  заморожена — ИММУННА к НОВОМУ Fear/Burn: on_attack-кейсы 'fear'/'burn' (abilities.js)
  проверяют `ctx.target.frozen` первым; AOE `fear_all`/`burn_all` и targeted-спеллы
  (doSpellFearTarget/doSpellBurnTarget + их клик-хендлер и подсветка в render.js) исключают
  замороженных из списка ВАЛИДНЫХ целей вообще (настоящий иммунитет, не просто fizzle —
  по прямому запросу автора, в отличие от Ward, который у targeted-спеллов именно fizzle).
- Ward блокирует наложение Frost целиком (та же формулировка, что у Fear/Burn — "Ward
  blocks the frost entirely").
- Заморозка — АНАЛОГ Solana Shield: следующий ЛЮБОЙ входящий удар (атака/контрудар/bolt/
  spell dmg/AOE) поглощается ЦЕЛИКОМ (`dmgCard()`, самый верх функции, тот же приоритет что
  у shield) и Заморозка снимается — с той же логикой "полностью поглощённый удар не несёт
  побочных эффектов" (`_frostBlockedThisHit`, аналог `_shieldBlockedThisHit`).
- НЕ снимается способностью Orbiton "Heal N and Clean" (onClick healTarget-ветка в game.js
  чистит только feared/burning/provokeBroken — card.frozen туда сознательно не добавлен) и
  НЕ снимается спеллом Dispel (та же логика — по прямому запросу автора только Clean назван
  явно, остальное консервативно не трогаем, пока автор не подтвердит).
- Полный ресет карты (смерть/возврат в руку/воскрешение) сбрасывает frozen/frozenTurnsLeft.
- Рендер: `mkSmallEl()` (render.js, только поле, как у Solana Shield) — оверлей-бокс
  `.frost-overlay` (css/styles.css), pop-in анимация при первом появлении, обратная
  (`.frost-leaving`) при снятии — снятие статуса ОТЛОЖЕНО на 300мс (`scheduleFrostRemoval()`
  в game.js), чтобы CSS успела доиграть анимацию исчезновения, тот же приём, что уже
  использует `.burning-out` в руке. Повторный pop-in на каждый рендер (DOM здесь
  пересоздаётся с нуля каждый тик) предотвращён отдельным Set (`_frostSeenIds` в render.js,
  тот же паттерн, что `_seenPcardPids`). Звука пока нет (по прямому запросу автора).

**Ещё 3 ультраредких Mood-трейта названы, но БЕЗ кода** (см. предыдущую запись сессии
2026-07-27 выше) — Foxy Skill (Orange from FFF), Saga (Saga from Krtv), Game of Market
(To the Moon with DHD).

**Новые карты:**
- TRAVELER #1049 (Tea, Xuiqtr, cost5): `frost`+`enter_draw:1` (Valley) → 6/3 по формуле
  Xuiqtr (HP=cost+1, ATK=cost-1, искл. cost≥5→cost-2).
- TRAVELER #1029 (Jeet, Szarg, cost4): `frost`+`incarnation:4` → 4/5 по формуле Szarg
  (HP=cost, ATK=cost+1).

**Проверено:** имя ассета у TRAVELER #1066 (см. правку номера 1068→1066 ранее в этой же
сессии) уже стояло `1066.png` — правка не нужна, было исправлено сразу при переименовании.

----
### Фикс рендера Заморозки + статус-панель (2026-07-27, тем же вечером)

**Баг найден автором:** frost.png не рендерился вообще, хотя ассет есть в репо и тег
`card.frozen` выставлялся корректно. Причина: `mkSmallEl()` (render.js) в середине функции
делает `d.innerHTML=\`...\`` (весь основной шаблон карты) — это ПОЛНОСТЬЮ заменяет содержимое
`d`, стирая любой `appendChild()`, сделанный ДО этой строки. Оверлей заморозки создавался и
добавлялся именно так — ДО этой перезаписи — и тут же стирался. **Тот же баг молча топит и
`inv` (Invisible-лейбл)** чуть выше по функции — просто это осталось незамеченным, т.к.
`.invisible-visual` и так стилизуется через CSS-класс без текста, не через сам appendChild'нутый
узел.

**Фикс:** оверлей заморозки теперь строится как HTML-строка (`frostBoxHtml`) ЗАРАНЕЕ и
вставляется ВНУТРЬ самого `d.innerHTML=` шаблона — тем же способом, что уже работают
`card-small-burning`/`-feared`/`-tauntbroken` чуть ниже по тому же шаблону. `inv`-баг НЕ
трогали (за рамками текущей задачи, отдельно на заметку — см. этот же абзац).

**Статус-панель (`_cardStatusEntries()`, render.js):** добавлена запись для Заморозки —
"Frozen for N more of its own turns — cannot act at all. Any incoming damage will be blocked
entirely and shatter the freeze." (иконка `ico_snow.png`), показывает актуальный остаток
`card.frozenTurnsLeft`.

----
### Fear/Burn против Frost/Ward/активного Solana Shield — истинный иммунитет без побочек (2026-07-27)

По прямому запросу автора — два изменения:

1. **Клик по иммунной цели больше не отменяет спелл.** Раньше клик по замороженной карте
   (targeted Fear/Burn спелл) попадал в общий `cancelPendingSpell()` — то же самое, что клик
   мимо поля. Теперь это тихий no-op: спелл остаётся pending, ждёт другую цель. Тот же принцип
   теперь и для Ward, и для карты с активным Solana Shield — раньше их можно было выбрать
   целью (клик проходил), эффект просто физзлил ВНУТРИ резолвера; теперь клик по ним тоже
   ничего не делает на уровне выбора цели (game.js, onClick(), spellBurnTarget/spellFearTarget
   ветки) — не долетают даже до резолвера. Подсветка (render.js, mkSmallEl) тоже перестала
   помечать такие карты как `targetable`.
2. **Solana Shield больше НЕ тратится попыткой наложить Fear/Burn** — ни точечно (было:
   `shieldConsumed=true` + "blocks... and shatters" в `doSpellFearTarget`/`doSpellBurnTarget`),
   ни по AOE (было то же самое в `fear_all`/`burn_all`, abilities.js). Теперь Shield ведёт себя
   как Ward — карта с активным щитом просто ИСКЛЮЧЕНА из списка валидных целей заранее, ничего
   не тратится и не ломается. **Только реальный входящий урон (`dmgCard()`) может снять
   Shield** — та же формулировка, что уже применяется к Frost.
   - `doSpellFearTarget`/`doSpellBurnTarget` (game.js) — упрощены до defensive-фолбэка (клик-
     хендлер уже фильтрует эти случаи выше, резолвер почти никогда не должен сюда доходить с
     иммунной картой), добавлено сообщение и для замороженных.
   - `fear_all`/`burn_all` (abilities.js) — фильтр целей расширен: `!hasTag(t,'ward')&&
     !t.frozen&&!(hasTag(t,'shield')&&!t.shieldConsumed)`. Сообщение при пустом списке
     целей изменено на "no valid targets — fizzles" (было "no enemy creatures").
   - ai.js — обновлены ВСЕ точки, где ИИ выбирает/оценивает Fear/Burn цели (targeted single +
     AOE, и точечная (spellFearTarget/spellBurnTarget click-эмуляция), и scoring
     (aiScoreCard), и предварительная проверка играбельности (aiSpellHasValidTarget)) —
     везде добавлено исключение активного Shield рядом с уже существующим Ward/Frost.

----
### Два бага, найденные автором вживую (2026-07-27, тем же вечером)

**1. Frost pop-in/pop-out анимация была практически невидима.** Причина: `frost.png` не был
в списке предзагрузки (`criticalImages`, ui.js) — на первом применении тега картинка ещё не
была в кеше браузера, а CSS-анимация scale(0.01→1) (`.frost-overlay`, css/styles.css)
стартует сразу по вычислению стилей, НЕ дожидаясь загрузки самой картинки. Скачивание
занимало дольше, чем сама анимация (0.25-0.3с) — к моменту, когда картинка прогружалась,
transform уже давно доехал до scale(1), пользователь видел только "внезапное появление уже
в полный размер". Фикс: `img/frost.png` и `img/ico_snow.png` добавлены в `criticalImages`
(ui.js) — грузятся заранее вместе со всеми остальными игровыми иконками, до первого
использования тега в бою.

**2. Intercept — открытый Xuiqtr не защищал ЗАКРЫТОГО Xuiqtr.** `getInterceptor()` (game.js)
содержал жёсткое правило с 2026-07-19 ("если target сам несёт intercept — перехвата вообще
нет"), задуманное как защита от воровства хитов МЕЖДУ Intercept-существами. Побочный эффект:
правило срабатывало ВСЕГДА, когда цель — Xuiqtr, включая случай, когда сама цель уже
exhausted/feared/frozen/уже использовала перехват в этот ход (т.е. реально "закрыта" и не
может сама перехватывать) — такая цель оставалась совсем без защиты, хотя рядом стоял
открытый Xuiqtr, который прикрыл бы любое ДРУГОЕ существо на этом же поле.
**Исправлено:** протекция другими Intercept-существами теперь пропускается, ТОЛЬКО если
сама цель прямо сейчас САМА способна перехватывать (`!target.exhausted && !target.feared
&& !target.frozen && !target.interceptUsed`) — то есть реально "стоит открытой" и в защите
не нуждается. Если цель закрыта — она обычная беззащитная цель, и другой открытый Xuiqtr
обязан её прикрыть, как и любую другую карту того же поля.

----
### Три правки (2026-07-27, тем же вечером)

**1. SCHEME (Tea) / BLACK MAGIC (Jeet)** — cost 2→4, `ess_add:4`→`ess_add:5` (+5 эссенции на
ТЕКУЩИЙ ход, essMax не растёт — тот же принцип, что и раньше, эффект временный), добавлен
`draw:1`. Новый текст: "Get 5 Essence and Draw 1."

**2. Orbiton — откат ATK на cost3 (1-1-2-2-2 → 1-1-1-2-2).** Порог ATK-исключения в формуле
сдвинут с cost≥3 на cost≥4 (по прямому запросу автора). Затронуты все 6 живых cost3-карт:
#10, #692, #454, #523, #420, #568 — hp не менялся, только atk 2→1. См. обновлённую формулу
в разделе "Adding a New Card" выше.

**3. Баг с тишиной при Fear/Burn-атаке по Ward/Frost-цели (автор поймал живьём).**
`willFear`/`willBurn` в `doAttack()` проверяли только тег атакующего + выживание цели, не
саму возможность дебаффа реально примениться — из-за этого атака Fear/Burn-существа по
Ward-цели (или по замороженной — тот же принцип) проходила В ПОЛНОЙ ТИШИНЕ: обычный звук
атаки пропускался (считалось, что сыграет debaf-звук), а debaf-звук не играл вообще, т.к.
Ward/Frost блокируют эффект без своего звука. Исправлено: добавлена проверка
`debuffBlocked = target.frozen || hasTag(target,'ward')` — если дебафф реально не наложится,
играет обычный звук атаки вместо тишины. Solana Shield НЕ включён в этот фикс намеренно —
блок щита по конвенции тихий (только лог+визуал, без звука) и на щит автор не жаловался;
трогать его звуковое поведение отдельно не просили.

----
### Новый тег `foxy` ("Foxy Trick") — второй ультралегендарный Mood-трейт, 2026-07-27

Orange from FFF (ультраредкий Mood, 5 шт. в коллекции) получил код: `foxy`, иконка
`ico_fff.png`. **Механика: 50% шанс промахнуться мимо ЭТОЙ карты полностью** — один бросок
на весь удар/спелл, решает урон И дебафф разом (не два независимых броска). Звука пока нет
(по прямому запросу автора, добавит позже) — только визуал (`queueFieldFx('MISSED!',
'fx-miss')`, нейтрально-серый попап, отличается от красного fx-fear/fx-spell-dmg).

**Порядок:** проверяется ПЕРВЫМ, до Frost/Shield/Armor/Ward (по прямому запросу автора) — на
промах ничего вообще не трогается (Frost/Shield не тратятся, промах = удар физически не
долетел).

**Что ловит уклонение:** любой входящий урон (`dmgCard()`, любой источник — атака/контрудар/
bolt/spell dmg/AOE) И попытка навесить дебафф — Fear, Burn, Frost, Provoke-break (spell-версия
и on_attack-версия обеих). Одна и та же карта в одной атаке либо ловит ВСЁ (промах), либо не
ловит НИЧЕГО (обычное разрешение).

**Что НЕ участвует:** burn-тик (мимо dmgCard(), отдельная механика), контрудар этой же карты
(когда она сама атакует и получает сдачи — это не "она как цель", а её собственный ответный
удар). "Уничтожение" отдельно не обрабатывается — в игре нет отдельного instant-kill без
урона (JUDGMENT/DEATHBLOW технически просто наносит 999 урона через dmgCard()) — уже покрыто
общим правилом.

**Реализация (флаг `_foxyDodgedThisHit`, тот же паттерн что `_shieldBlockedThisHit`/
`_frostBlockedThisHit`):**
- `dmgCard()` (game.js) — сам бросок, самая первая проверка функции.
- on_attack `fear`/`burn`/`frost`/`taunt_break` (abilities.js) — проверяют флаг вместе с
  `_shieldBlockedThisHit` (НЕ добавлен в `scheme`-кейс — снятие баффов не считается
  дебаффом под уклонение, по прямому запросу автора).
- `doSpellFearTarget`/`doSpellBurnTarget`/`doSpellProvokeBreakTarget` (game.js) — эти НЕ идут
  через dmgCard() вообще (чистое навешивание статуса), бросок кидается заново, тем же
  способом, прямо в резолвере.
- `fear_all`/`burn_all` (abilities.js) — КАЖДАЯ цель со своим `foxy` кидает СВОЮ монетку
  независимо внутри `forEach` (в отличие от Ward/Frost/Shield — детерминированных
  иммунитетов, отфильтрованных заранее из списка целей).
- `doAttack()` (game.js) — звук атаки теперь решается ПОСЛЕ `dmgCard()` (было ДО — пришлось
  переставить, т.к. флаг промаха появляется только внутри dmgCard()): на промах — без звука
  вообще (только визуал, уже отыгран самой dmgCard()), иначе прежняя логика Ward/Frost-фикса
  из предыдущей записи сессии.
- `img/ico_fff.png` добавлен в предзагрузку (`criticalImages`, ui.js) — та же превентивная
  мера, что и у `frost.png` (см. запись про баг с невидимой анимацией выше).

----
### Новые карты с Foxy Trick + визуальный оверлей Invis/Stealth (2026-07-27)

**Новые travelers:**
- TRAVELER #24 (Tea, Szarg, cost4): `foxy`+`death_heal:4` (Bamboo) → 4/5 по формуле Szarg.
- TRAVELER #15 (Jeet, Umbasir, cost5): `foxy`+`enter_lose:1` (Net) → 5/2 по формуле Umbasir
  (HP=cost, ATK=1, искл. cost≥4→ATK=2).

**Визуальный оверлей Invis/Stealth (по прямому запросу автора) — заполняет плейсхолдер,
пустовавший с 2026-07-19** (см. старый комментарий у `.invisible-visual`/`.stealth-visual` в
css/styles.css — "пока нет отдельного визуального решения... любой будущий индикатор
продолжит работать"). Заодно найден и исправлен тот же баг, что раньше ловили на Frost:
`inv` (текстовая плашка "Invis") создавалась через `appendChild()` ДО `d.innerHTML=...`
дальше по `mkSmallEl()` — полностью стиралась, никогда реально не рендерилась. Убрана,
заменена на HTML-строку `invisBoxHtml`, вставляемую внутрь самого шаблона (тот же паттерн,
что и `frostBoxHtml`).

- Показывается (поле, `mkSmallEl()`, только там — не в руке/каталоге/деккбилдере, тот же
  scope, что у Frost-оверлея): пока Stealth ещё не раскрылась (`hasTag(card,'stealth') &&
  !card.stealthBroken`), ИЛИ пока Invisible и рядом на своём поле есть хотя бы один ВИДИМЫЙ
  (не-invisible) союзник — та же логика видимости, что уже использует
  `getTargetableCards()`/`allInvisible` (game.js): если все свои существа invisible разом,
  сама карта уже targetable, оверлей не нужен.
- Картинка `img/invis.png` — **тот же размер, что и сама card-small (100%, НЕ 110%, в отличие
  от Frost)**, прозрачность уже в самом ассете, доп. эффектов не добавляли.
- z-index между обычным содержимым карты и дебафф-бэйджами/Frost-оверлеем — "поверх всего,
  кроме дебафов и фрост ассета", по прямому запросу автора: card-small-feared/-burning/
  -tauntbroken (z-index:5) и .frost-overlay (z-index:20) рисуются НАД этим боксом (z-index:3).
- Появляется/исчезает мгновенно вместе с обычным `render()` — отдельной анимации не просили
  (в отличие от Frost pop-in/pop-out).
- `img/invis.png` добавлен в предзагрузку (`criticalImages`, ui.js) — та же превентивная мера,
  что и у `frost.png`/`ico_fff.png` (см. запись про баг с невидимой анимацией выше).

----
### Три бага, найденные автором вживую (2026-07-27, тем же вечером)

**1. Frost pop-out (исчезновение) не проигрывался — бокс пропадал мгновенно.** Причина —
`scheduleFrostRemoval()` (game.js): `card.frozen` становился `false` ТОЛЬКО внутри
`setTimeout`, ОДНОВРЕМЕННО с `_frostLeaving=false` — состояние "уже не frozen, но ещё
`_frostLeaving=true`" (единственное, при котором `mkSmallEl()` вообще применяет класс
`.frost-leaving` и проигрывает CSS-анимацию сжатия) физически ни разу не рендерилось.
Появление (frostIn) работало нормально — баг был именно в асимметрии тайминга. Исправлено:
`card.frozen=false` теперь выставляется СРАЗУ, синхронно с `render()`, а `setTimeout` дальше
только ждёт конца самой CSS-анимации (0.3с) и убирает бокс из DOM насовсем.

**2. Мишень (target reticle, mishen_red/green.png) рисовалась ПОД Frost-оверлеем.**
`.card-small.aim-target::after`/`.aim-heal::after` имели z-index:5 — ниже
`.frost-overlay` (z-index:20), из-за чего на замороженной цели мишень выглядела блёкло.
Поднято до z-index:25 — теперь мишень поверх всего на карте, как и дебафф-бэйджи (по
прямому запросу автора).

**3. Полная сверка предзагрузки картинок.** Помимо `ico_cloud.png` (Thunder Storm, которую
автор заметил напрямую), найдено ЕЩЁ 12 давно существующих тег-иконок, которые никогда не
были в списке `criticalImages` (ui.js) вообще — гэп существовал ДО этой сессии, не из-за неё:
`ico_bambo`, `ico_fire_shield`, `ico_haunt`, `ico_incarn`, `ico_intercept`, `ico_invis`,
`ico_remember`, `ico_scheme`, `ico_solana_shield`, `ico_stealth`, `ico_untamed`, `ico_ward`.
Все добавлены в предзагрузку — та же потенциальная "невидимая анимация/задержка при первом
использовании", что уже чинили у Frost, теоретически могла касаться и их.

----
### Расследование "вылета на лендинг" в хотсите (2026-07-27, вечер)

Автор словил дважды за сессию (и один раз раньше) необъяснимый вылет прямо на лендинг в
момент передачи хода через Spacebar, В КОНСОЛИ НИ ОДНОЙ ОШИБКИ ни разу — и не мог поймать
лог в момент краша (страница просто перезагружается/сбрасывается).

**Вероятная причина (не подтверждена вживую, но объясняет ВСЕ симптомы разом):**
Spacebar-хендлер (game.js) слепо жал `confirmYesBtn` в `#confirmModal`, если тот оказывался
не скрыт (`!modal.classList.contains('hidden')`) — а этот ЖЕ модал переиспользуется для
`askMenu()` ("← Menu" кнопка в игре и в hamburger-меню → "Yes, Exit" → `showLanding()`,
полный сброс игры) И `askRestart()` ("Yes, Restart"). Если игрок хоть раз машинально/
случайно ткнул "← Menu" (кнопка сидит прямо в углу игрового экрана) и не заметил/не закрыл
модалку Cancel'ом — модал оставался открытым, и СЛЕДУЮЩЕЕ нажатие Space "чтобы передать ход"
молча ПОДТВЕРЖДАЛО ВЫХОД вместо этого. Это объясняет отсутствие ошибок в консоли — код
отрабатывает штатно и корректно, просто не от того нажатия.

**Фикс:** `#confirmModal` полностью убран из Spacebar-шорткатов (game.js) — теперь Space по
этому модалу вообще ничего не делает (глушит только скролл). Дестрактивные действия (выход/
рестарт) подтверждаются ТОЛЬКО явным кликом/тапом. Остались доступны через Space:
`winModal`, `mulliganScreen`, `passScreen` (device-handoff Ready) — все три безопасны/
не деструктивны.

**Дополнительный слой (на случай, если причина всё же где-то в необработанном исключении,
а не в этом сценарии) — персистентный crash-лог (index.html, первый script на странице):**
`window.addEventListener('error'/'unhandledrejection', ...)` пишет детали (message/stack/
timestamp/URL) в `localStorage` СРАЗУ в момент возникновения — переживает даже полную
перезагрузку страницы. При СЛЕДУЮЩЕЙ загрузке, если есть сохранённый краш, он громко
печатается в консоль автоматически (`console.error`) — можно скопировать и прислать, даже
если консоль не была открыта в момент самого краша. Также доступно в любой момент из консоли:
`getLastCrash()` / `clearLastCrash()`.

**Если проблема повторится** — первым делом стоит проверить (1) не был ли в тот момент
случайно открыт `#confirmModal` (можно спросить автора, не мелькала ли надпись "LEAVE GAME?"
до вылета), и (2) вызвать `getLastCrash()` в консоли сразу после перезагрузки — если что-то
там есть, это уже настоящее необработанное исключение, а не сценарий с confirmModal выше.

----
### Три бага с Frost/Ward-таргетингом, найденные автором вживую (2026-07-27, ночь)

**1. Frost повторно накладывался на уже замороженную цель в момент, когда удар должен был
полностью поглотиться.** Ни один из on_attack-кейсов (burn/fear/frost/taunt_break/scheme,
abilities.js) не проверял `_frostBlockedThisHit` — только `_shieldBlockedThisHit`/
`_foxyDodgedThisHit`. Атакующий с тегом `frost`, бьющий по УЖЕ замороженной цели: dmgCard()
корректно поглощает удар целиком (см. историю Frost выше), но следом on_attack-кейс 'frost'
не видел флага поглощения и вешал Заморозку ЗАНОВО — хотя по дизайну удар должен был
полностью пропасть без следа. Исправлено — добавлена проверка `!ctx.target._frostBlockedThisHit`
во все 5 мест (burn/fear/frost/taunt_break/scheme). Solana Shield уже работал верно — та же
проверка `_shieldBlockedThisHit` там была с самого начала.

**2. Побочный баг, найденный по пути: `_frostBlockedThisHit` вообще никогда не сбрасывался**
в начале `dmgCard()` (в отличие от `_shieldBlockedThisHit`/`_foxyDodgedThisHit`, которые
сбрасываются). Один раз выставленный в `true`, флаг мог навсегда "залипнуть" и тихо глушить
debuff-эффекты на ВСЕХ последующих, никак не связанных ударах по этой же карте. Добавлен
сброс в самое начало функции.

**3. Targeted damage-эффекты позволяли выбрать Ward/Frost/активный Shield как цель, играя
анимацию (HIT!/BOLT!/SHARD!/DESTROYED) без реального эффекта — по прямому запросу автора,
тот же принцип, что раньше применили к Fear/Burn:**
- `spellDmgTarget`, `spellExecuteHalfTarget`, `spellDmgTrampleTarget` (targeted-спеллы) —
  Ward/Frost/активный Shield теперь полностью исключены из выбираемых целей (клик +
  подсветка в render.js + AI-таргетинг и aiSpellHasValidTarget в ai.js).
- **Заодно распространено на Shard и Bolt** (ручной выбор цели активкой, не спелл технически,
  но та же самая проблема) — та же трёхкратная защита (клик/подсветка/AI, Ward у AI уже был
  раньше, добавлены Frost/Shield).
- **SCATTERSHOT/SHRAPNEL (`random_spread`, abilities.js)** и **Board Purge (`aoe_count`)** —
  тут логика ДРУГАЯ: цели случайны, Ward/Frost/Shield НЕ исключаются из пула (урон по
  дизайну честно может улететь впустую в защищённую цель, это не баг) — но `HIT!`-анимация
  теперь не проигрывается для той конкретной попавшей под раздачу защищённой цели.

**Побочный найденный и исправленный на месте баг (при работе над п.3):** в процессе правки
`aiTryUseShard()` (ai.js) по невнимательности случайно удалил `if(enemyField.length===0)
return false;` и `const baseDmg=shardBaseDmg(...)` — заметил и восстановил в этой же сессии,
до сборки финального архива.

----
### Расследование "вылета на лендинг", раунд 2 — вероятная НАСТОЯЩАЯ причина найдена (2026-07-27, ночь)

Автор словил краш снова, УЖЕ ИСКЛЮЧИВ теорию с confirmModal из прошлой записи (в этот раз
диалог "LEAVE GAME?" точно не был открыт). `getLastCrash()` в консоли — "no logs saved", т.е.
это НЕ обычное JS-исключение (window.onerror молчал).

**Проверка через симулятор (`sim/hunt-crash.js`, новый файл — headless AI vs AI, тот же
движок что и в браузере, без DOM):** 4000 партий подряд со случайными Rush-колодами
(отдельный прогон specifically колодами ТОЛЬКО из карт с новыми тегами этой сессии —
frost/foxy/death_armor/death_bolt/remember/rage/vampiric/incarnation/enter_lose/enter_draw/
vanguard/shield/ward/intercept/untamed/draw_attack) — **0 крашей**. Это сильно снимает
подозрение с самой игровой логики (data.js/abilities.js/game.js/ai.js) — движок стабилен даже
на экстремальных наборах карт.

**Уточнено у автора:** устройство — iPad/MacBook, **Safari** (оба раза); краш **только после
долгой сессии** (не за пару минут); **всегда во время активного матча** (не в деккбилдере).

**Вероятная НАСТОЯЩАЯ причина: Safari тихо убивает и перезагружает вкладку под давлением
памяти за долгую сессию — это НЕ баг в игровой логике, а архитектурная особенность рендера +
типичное поведение WebKit.** Подтверждающие находки:
- `preloadAssets(); initState(); ...` (boot-секция, ui.js, самый низ) выполняется БЕЗУСЛОВНО
  при любой загрузке страницы — `#landing`/`#game` оба стартуют с `display:none` в разметке
  (index.html), лендинг показывается ТОЛЬКО через этот boot-код. Значит: если Safari просто
  тихо перезагрузит вкладку (без единой JS-ошибки — это нормальное поведение ОС/браузера при
  нехватке памяти, не что-то, что можно поймать через `window.onerror`), страница на 100%
  всегда взойдёт заново именно на лендинг — ровно то, что описывает автор, и ровно потому лог
  не сохраняется (ошибки никогда и не было).
- Архитектура рендера (`render()`, render.js) ПОЛНОСТЬЮ пересоздаёт весь DOM карт на КАЖДЫЙ
  вызов (mkSmallEl()/mkEl() всегда строят новые узлы, не переиспользуют старые) — за долгую
  партию (много ходов/атак/спеллов) это ощутимый источник GC-мусора.
- **Найден и исправлен конкретный, реальный источник O(n²)-роста за долгую партию:** `lg()`
  (state.js) на КАЖДЫЙ вызов перестраивал `innerHTML` ВСЕЙ панели лога заново из `G.logs`
  целиком (`.filter().map().join('')`) — то есть каждая НОВАЯ строчка лога заново
  перерисовывала ВСЮ накопленную историю партии. За долгую партию (сотни записей: бои,
  спеллы, триггеры, снапшоты ходов) — квадратичный рост работы и мусора на каждый чих.
  **Исправлено:** `lg()` теперь ДОБАВЛЯЕТ только новую строку (`appendChild` одного узла)
  вместо пересборки всего — `G.logs` НЕ обрезаем (полная история по-прежнему нужна для
  `downloadBattleLog()`), меняется только рендер видимой панели. `initState()` теперь явно
  очищает DOM-панель лога при старте новой партии (раньше это неявно происходило само —
  побочный эффект полной пересборки, которой больше нет).

**Дальнейшие шаги, если проблема повторится:**
1. Попробовать НЕ в приватном режиме Safari (Private Browsing на iOS обычно строже по лимитам
   памяти и агрессивнее убивает вкладки — на скриншоте раньше в этой же сессии вкладка была
   именно "Private").
2. Проверить, реже ли случается краш ПОСЛЕ фикса `lg()` выше — если частота заметно упадёт,
   это подтвердит теорию.
3. Если так и не исчезнет — рассмотреть более глубокую оптимизацию: инкрементальный diff/patch
   рендера карт вместо полной пересборки DOM на каждый `render()` (более крупная работа,
   отдельная задача, не в рамках этой сессии).

**Новый файл `sim/hunt-crash.js`** — утилита для будущих похожих расследований: строит
случайные Rush-колоды (с упором на конкретные теги через `PRIORITY_TAGS`) и гоняет их через
`headless.js` `runGame()`, ловя любое необработанное исключение сразу в stdout/stderr —
удобно натравливать на новые карты/теги перед тем, как автор сам сядет их тестировать вручную.

----
### Откат ошибки: Shield/Frost должны ОСТАВАТЬСЯ целями для урона (2026-07-27, ночь)

Автор поймал мою ошибку в предыдущей правке ("targeted damage-эффекты не дают выбрать
Ward/Frost/Shield"): для эффектов УРОНА это было неверно для Shield и Frost — их МЕХАНИКА
ИМЕННО в том, чтобы ловить урон и лопаться/сниматься. Их нужно уметь выбирать целью урона
(щит/заморозка абсорбируют и снимаются), а вот Ward — это НАСТОЯЩИЙ иммунитет к магическому
урону (не ловит, не снимается, урон просто не проходит) — вот его действительно не стоит
давать выбрать целью урона, чтобы не было "анимация есть, эффекта нет".

**Откачено (только Ward остался исключён, Shield/Frost снова доступны для урона):**
`spellDmgTarget`, `spellExecuteHalfTarget`, `spellDmgTrampleTarget`, `shardTarget`,
`boltTarget` — клик-хендлеры (game.js), подсветка (render.js), AI-таргетинг и
`aiSpellHasValidTarget`/`aiTryUseShard`/`aiTryUseBolt` (ai.js). 13 мест правки суммарно.

**НЕ трогалось (осталось верно, как и было задумано с самого начала)** — Fear/Burn (спеллы
и AOE) по-прежнему исключают Ward+Shield+Frost все втроём: это ДЕБАФФЫ, не урон, у них
другая природа (иммунитет, не "ловит и снимается").

**Тонкий случай, который автор прояснил и который получается АВТОМАТИЧЕСКИ из чистой
Ward-проверки, без доп. кода:** карта с ОБОИМИ тегами (`ward`+`shield`) — пока щит ещё
активен, её МОЖНО выбрать целью урона (щит поглотит и снимется); как только щит потрачен —
её уже НЕЛЬЗЯ (Ward теперь защищает тело напрямую). Это следует просто из того, что
проверяется ТОЛЬКО `hasTag(card,'ward')`, независимо от состояния Shield — щит истории не
играет роли в этой конкретной проверке.

----
### РАЗГАДКА "вылета на лендинг" — автор поймал сам (2026-07-27, ночь #3)

**Настоящая причина, наконец найдена (автором лично, не багой в коде вообще):** это была
привычка нажимать Space ДВАЖДЫ подряд — один раз, чтобы закончить ход, и сразу второй раз
"на автомате", чтобы подтвердить pass-device экран ("передать устройство"). Когда игра
заканчивалась по истощению колоды (fatigue win, 3 неудачных попытки взять карту), вместо
pass-device экрана внезапно всплывал WIN MODAL — и то самое "второе" нажатие Space, сделанное
не глядя по привычке, тут же ЕГО закрывало. Автор не успевал заметить победный экран и
считал, что это краш. Все предыдущие расследования (confirmModal, симулятор на 4000 партий,
теория про Safari/память, оптимизация lg()) были добросовестными, но били не в ту точку —
реальная причина не имела отношения ни к JS-исключениям, ни к производительности.

**Фикс:** Space теперь работает ТОЛЬКО для двух вещей — (1) конец хода, когда никакой модал
не открыт, и (2) кнопка "Ready" на pass-device экране ("передать устройство"). `winModal` и
`mulliganScreen` полностью убраны из списка Space-шорткатов — по ним Space ничего не делает
(и явно НЕ проваливается дальше в endTurn(), см. отдельный guard). Победный экран и муллиган
теперь требуют осознанного клика/тапа — больше никогда не проскочат мимо незамеченными.

**Судьба побочных находок этой сессии:** остаются в силе как самостоятельные, реальные
улучшения, даже если не были причиной этого конкретного бага — `lg()`-оптимизация (O(n²)→O(n)
на партию), персистентный crash-логгер (`getLastCrash()`), `sim/hunt-crash.js` (утилита для
будущей охоты за багами через симуляцию), и исправление confirmModal (там действительно была
реальная, отдельная опасность — деструктивное действие по Space, просто не эта).

----
### Геймдизайн-фикс: fatigue-проигрыш стал заметным (2026-07-27, ночь #4)

Автор верно заметил: раз даже ЕМУ было непонятно, откуда взялся "краш" (см. предыдущую
запись — на деле пропущенный победный экран), то будущим игрокам при презентации игры
происходящее при истощении колоды будет совсем непонятно. Три связанных правки:

**1. Счётчик колоды мигает красным при 0 карт.** `render()` (render.js) теперь навешивает
класс `.deck-count-empty` на оба счётчика колоды (`#{f}DeckCountStat` в статус-баре и
`#{f}DeckBadge` в нижней панели) при `p.deck.length===0`. Анимация — тот же `mishenBlink`
(1.1с, "не сильно быстрое" мигание), что уже используют мишень атаки и `.pcard.aim-target`.

**2. Растущая надпись-предупреждение по центру экрана.** `_playBattleBeginsText()` (ui.js)
обобщена в `_playCenterBannerText(text)` — та же анимация (вырастает из шва между полями боя
→ держится/пульсирует 1с → fade, ~2с всего), просто с произвольным текстом вместо
захардкоженного "Battle begins!". Новая `showDeckEmptyWarning(turnsLeft)` показывает
"NO CARDS LEFT! You lose in N more turn(s)!" — вызывается из `_runTurnStartEffects()`
(game.js) сразу после неудачной попытки добора, ДО того как 3-я попытка добьёт до
поражения (тогда вместо баннера сразу всплывает победная модалка).

**3. Разделён `endTurn()` на "конец старого хода" (сразу) и "начало нового"
(`_runTurnStartEffects()`, новая отдельная функция) — геймдизайн-баг, тоже пойманный
автором.** Раньше В ХОТСИТЕ весь "старт хода" (тик горения/инкарнации, эссенция, добор
карты + fatigue-предупреждение, экран "Turn N") отыгрывался СРАЗУ при нажатии end-turn — то
есть ПОКА на экране висел pass-device оверлей "передайте устройство". Игрок слышал звуки
горения/лечения/т.п., но не видел ничего, а предупреждение о пустой колоде (п.2 выше) пряталось
за тем же оверлеем, оставаясь незамеченным ИМЕННО в тот момент, когда оно нужнее всего.
**Теперь в хотсите:** `endTurn()` сразу переключает `G.turn` (флип хода, снятие
sleeping/feared/frost-тик у выходящего игрока — всё, что не раскрывает новую информацию), но
сам показ pass-device экрана происходит ДО начала-хода-эффектов, а не после — `showPassScreen()`
получает `_runTurnStartEffects` в качестве `onReady`-колбэка (тот же механизм, что уже
использовался для муллиган-хэндоффа tea→jeet и в деккбилдере) и запускает его ТОЛЬКО когда
следующий игрок нажимает "Ready". В vs-AI/AI-vs-AI spectator хэндоффа нет вообще — там
`_runTurnStartEffects()` запускается сразу, как и раньше, без задержки.

Проверено симулятором (`sim/headless.js` + `sim/hunt-crash.js`, 800 партий суммарно,
0 крашей) — рефакторинг не задел игровую логику, только тайминг показа в хотсите.

----
### Четыре правки по деталям Ward/Shield/Frost/destroy (2026-07-27, ночь #5)

**1. Мигание счётчика колоды исправлено** (автор уточнил формулировку) — раньше мигала
opacity ВСЕГО элемента, включая background-image "экран дисплея" у `.stat-badge` — выглядело
неестественно ("экран становится прозрачным"). Теперь мигает ТОЛЬКО цвет/свечение самой
цифры (`color`+`text-shadow`, фон это не затрагивает), плюс у бэйджа с реальным экраном
(`.stat-badge.deck-count-empty`) добавлен СТАТИЧНЫЙ (не мигающий) красный оттенок+свечение
самого бокса (`box-shadow: inset ... `).

**2. TRAVELER #250 — cost 5→4** (по прямому запросу автора: Szarg вообще не должен
существовать на cost5), hp/atk пересчитаны по формуле Szarg = 4/5. Других cost5 Szarg в
data.js не найдено.

**3. Исправлена логика Ward+Shield для damage-таргетинга (targeted-урон/Bolt/Shard/
Execute-Half).** Раньше (после отката в предыдущей сессии) условие проверяло только
`hasTag(card,'ward')` — но Ward это ПОСТОЯННЫЙ тег, а Shield ВРЕМЕННЫЙ (тратится) — карта с
ОБОИМИ тегами сразу оказывалась вообще недоступна для урона, хотя щит ещё цел и должен ловить
удар. Исправлено на `hasTag(card,'ward') && !(hasTag(card,'shield')&&!card.shieldConsumed)` —
Ward блокирует таргетинг, ТОЛЬКО если не прикрыт ещё активным Shield (щит — слой ЗАЩИТЫ ВЫШЕ).
13 мест (5 click-хендлеров game.js, 5 подсветок render.js, 9 мест в ai.js).

**4. Frost больше не спасает от DESTROY-эффектов + добавлен попап IMMUNE.**
- Новый параметр `bypassFrost` у `dmgCard()` (7-й аргумент) — когда true, пропускает
  Frost-абсорб полностью (Shield/Ward по-прежнему проверяются как обычно СРАЗУ после).
  Статус Заморозки при этом не трогается — если карта переживёт этот конкретный удар,
  просто останется замороженной как была.
- `doSpellExecuteHalfTarget` (JUDGMENT/DEATHBLOW) — оба удара (Bolt 1 + добивающие 999)
  теперь идут с `bypassFrost=true`.
- `destroy_all_enemies` (CATACLYSM/EXTINCTION, AOE) — та же логика: Ward блокирует (с учётом
  ещё-активного Shield, см. п.3), Frost не спасает вообще. На Ward-заблокированной цели
  теперь всплывает **IMMUNE** (`fx-immune`, голубой) вместо DESTROYED, и `dmgCard()` для
  неё вообще не вызывается.
- `fear_all`/`burn_all` — переписаны на итерацию по ВСЕМ врагам (было: тихая фильтрация
  списка целей заранее). Иммунные (Ward/Frost/активный Shield) теперь показывают **IMMUNE**
  вместо полной тишины — понятнее, почему эффект не сработал на конкретной карте.

Проверено симулятором (700 партий, 0 крашей) — правки не задели игровую логику вне
Ward/Shield/Frost-взаимодействий.

----
### IMMUNE/ABSORB — доводка анимаций (2026-07-27, ночь #6)

По прямому запросу автора, три доп. места:

1. **SCATTERSHOT/SHRAPNEL (`random_spread`)** — случайное попадание в Ward-цель теперь
   показывает IMMUNE (было: тихо, без анимации). Frost/активный Shield по-прежнему без
   отдельной плашки здесь — у них своя, изнутри `dmgCard()` (см. п.3 ниже).
2. **Board Purge (`aoe_count`)** — тот же фикс, зеркально.
3. **Solana Shield — плашка ABSORB** (была задумана раньше, потерялась по пути — автор
   вспомнил, восстановлено). Новая `queueFieldFxReplace(cardId,label,cls)` (game.js) —
   обычный `queueFieldFx()` тут не годится: звонящий код (`doAttack()`/`doBoltTarget()`/
   `doSpellDmgTarget()` и т.п.) почти всегда СНАЧАЛА ставит в очередь свою "HIT!"/"BOLT!"
   плашку и только ПОТОМ зовёт `dmgCard()` — обе плашки легли бы друг на друга нечитаемо.
   `queueFieldFxReplace()` вместо этого вычищает уже поставленные (но ещё не отрисованные)
   fx-записи для той же карты перед тем как добавить новую — гарантированно остаётся только
   ABSORB, ничего не наслаивается. Вставлено в shield-блок `dmgCard()`.
   Цвет — золотой (`fx-absorb`, тот же тон, что у `fx-squad`).

AOE Fear/Burn (fear_all/burn_all) IMMUNE — уже было сделано в предыдущей записи сессии,
...
----
### draw_attack + Thorns — обдумано и исправлено вместе с автором (2026-07-27, ночь #8)

Геймдизайн-разбор ПЕРЕД кодом (по прямому запросу автора): что должно происходить с
on_attack-эффектами, когда сама атака промахнулась (Foxy Trick) или была поглощена
(Solana Shield/Frost)?

**Философия, закреплённая в этой сессии:**
- **Промах (Foxy)** — атаки физически не было вообще, ничего из того, что она несла с собой,
  не случается. Уже верно для Fear/Burn/Frost/Taunt Break/Scheme; теперь то же самое для
  draw_attack и Thorns (см. ниже).
- **Поглощение (Shield/Frost)** — атака СОСТОЯЛАСЬ, атакующий реально замахнулся и ударил,
  просто магия цели свела последствия к нулю. Эффекты, завязанные на самом ФАКТЕ атаки
  (не на её последствиях для цели), должны срабатывать как обычно.
- Разные on_attack-эффекты закономерно принадлежат к разным категориям — это НЕ
  непоследовательность, а разный дизайн-смысл:
  - `draw_attack` (Ryvlen) и Stealth reveal bonus (+3 карты) — завязаны на самом факте атаки,
    работают и на поглощении. Разница между ними: draw_attack ВСЁ ЖЕ блокируется промахом
    (см. ниже), Stealth-бонус — нет вообще (это свойство раскрытия из стелса, не атаки).
  - `vampiric` — уже был завязан на `ctx.realDmgDealt>0` (намеренно, реальный урон нужен для
    "высасывания крови") — не работает НИ на промахе, НИ на поглощении. Не трогали.

**Правки:**
1. `draw_attack` (case 'draw', abilities.js) — добавлена проверка
   `ctx.target._foxyDodgedThisHit` — не тянет карту на промахе, ТЯНЕТ при поглощении
   (Shield/Frost) как и раньше.
2. **Thorns/Fire Shield** (doAttack(), game.js) — автор поймал живьём тот же класс
   недосмотра, что уже чинили у draw_attack/frost: проверял только `_shieldBlockedThisHit`,
   пропустив `_frostBlockedThisHit` и `_foxyDodgedThisHit`. Добавлены оба — шипы больше не
   бьют в ответ, если атака промазала мимо шипастой цели или была поглощена её же
   заморозкой (удар физически не долетел до тела — бить в ответ нечему).

Проверено симулятором (550 партий, 0 крашей).

----
### Дубли текста при DESTROY + мишень-череп для JUDGMENT/DEATHBLOW (2026-07-27, ночь #9)

**1. Двойной текст при Foxy-промахе на destroy-эффектах.** Тот же класс бага, что уже чинили
у Solana Shield (ABSORB) — почти все DESTROY-вызовы `dmgCard()` (VERDICT/DAMNATION/JUDGMENT-
добивание/CATACLYSM/EXTINCTION) ставят СВОЮ плашку "DESTROYED" в очередь ДО вызова, а сам
Foxy-бросок решается только ВНУТРИ dmgCard() — если сработал промах, под "MISSED!" всё ещё
было видно "DESTROYED" снизу. Исправлено: foxy-блок в `dmgCard()` теперь тоже использует
`queueFieldFxReplace()` (была введена для ABSORB чуть раньше в этой же сессии) — стирает
любую уже поставленную плашку перед тем как поставить MISSED!, гарантированно остаётся одна.

**2. Двойной "DESTROYED" при обычном (не промахнутом) уничтожении.** Автор поймал: рядом с
HP всплывала ЕЩЁ ОДНА надпись "DESTROYED", дублируя центральную. Причина —
`forceLabel`-параметр `dmgCard()` изначально (2026-07-24) был задуман показывать "DESTROYED"
именно ТАК, плавающим текстом у HP через `showFloat()`; позже (эта сессия) почти все
DESTROY-вызовы ДОПОЛНИТЕЛЬНО стали сами ставить большую центральную плашку тем же текстом
(тот же паттерн, что у HIT!/BOLT!/MISSED!/IMMUNE/ABSORB) — получались две надписи разом.
Исправлено: `showFloat()` теперь вызывается ТОЛЬКО когда `forceLabel` не задан (обычные
"-N" по-прежнему всплывают у HP как раньше) — при DESTROY-эффектах остаётся только
центральная плашка.

**3. Мишень-череп для JUDGMENT/DEATHBLOW/VERDICT/DAMNATION.** Новый CSS-класс
`.card-small.aim-target.aim-target-kill::after` — та же красная мишень, но картинка
`mishen_red2.png` (череп) вместо `mishen_red.png` — сигнализирует ЗАРАНЕЕ, что именно ЭТА
цель умрёт от каста. Логика в `mkSmallEl()` (render.js):
- **spellDmgTarget** (VERDICT/DAMNATION) — `getTagVal(spell,'spell_dmg_target')>=card.hp` —
  на практике insta-kill спеллы (999 dmg) всегда убивают любую валидную цель, череп
  поголовно на всех подсвеченных.
- **spellExecuteHalfTarget** (JUDGMENT/DEATHBLOW) — `(card.hp-1)<=Math.floor(card.maxHp/2)`
  — та же формула, что в самом резолвере: спелл сперва бьёт Bolt 1, добивает только если
  ЭТО опускает цель до половины maxHp — череп только на целях, где добивание реально
  сработает, обычная мишень на тех, кто просто получит чип-урон и переживёт.
