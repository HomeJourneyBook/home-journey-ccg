...
|`armor:N`      |Extra HP-like buffer, N points, absorbed BEFORE hp on any damage (attack/counter/AOE/shard/burn — all funnel through `dmgCard()`, see there). Refills to N at the start of the OWNER's own turn (`endTurn()`), NOT the opponent's. Set on enter (`doCreature()`) and reset to 0 when a card leaves field/gets reshuffled (`resetC()`). Added 2026-07-10, not yet on any live card — see Version 1.01.|
|`untamed`      |"Неукротимость" — this creature's `exhausted` clears already when ITS OWN turn ends (i.e. already usable/counter-attacking during the opponent's turn), instead of waiting for its owner's next turn like every other creature. Deliberate override of the normal exhausted-clears-on-owner's-turn rule (`endTurn()`) — Mood trait justification: Anime pink (see Lore/Trait mapping). Added 2026-07-10, not yet on any live card.|
...
- `untamed` (Anime pink) — реализован в движке 2026-07-10 (см. Tag System + roadmap выше),
  но пока не назначен ни одной живой карте.
...
- [x] Механика «Броня» — реализовано 2026-07-10. Тег `armor:N`, отдельный пул `card.armor`
  поверх HP: любой урон (атака/контратака/AOE/shard/targeted-spell/burn-тик — вообще всё, что
  идёт через `dmgCard()`, плюс отдельно продублировано для burn-тика в `endTurn()`, т.к. у
  него свой особый death-to-void путь мимо `dmgCard()`) сначала вычитается из `armor`, и
  только остаток (если есть) — из HP. Обновляется до полного N в начале хода ВЛАДЕЛЬЦА
  (`endTurn()`, тот же блок, что снимает exhausted владельцу), не хода соперника. Инициализация
  при входе на поле — `doCreature()`; сброс в 0 при возврате карты в колоду/руку — `resetC()`
  (state.js). Пока НЕ висит ни на одной живой карте — ждём, на кого автор его повесит
  (см. Trait mapping, Броня подходит и под 1/1, и под определённые Mood/World трейты).
  Рендер/иконка щита — сознательно НЕ делали в эту сессию, автор распишет отдельно.
- [x] Механика «Неукротимость» (`untamed`, working name, Anime pink Mood — см. Trait mapping) —
  ...
  (`endTurn()`, блок для ВЫХОДЯЩЕГО игрока). Пока НЕ висит ни на одной живой карте.
