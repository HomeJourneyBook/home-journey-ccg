...
- [ ] **Зеркалирование X/Y для остальных декоративных плейсхолдеров** (статус-бары уже
  ...
  виде справа.
- [ ] **Кнопка "назад" во всех модалках до муллигана** — цепочка landing → deckPickerModal →
  vsAiPickerModal/deckBuilderModal → mulliganScreen идёт только вперёд, выйти можно только
  через закрытие/рестарт. Нужна кнопка назад на каждом шаге (включая сам муллиган — назад к
  выбору деки/фракции). Не уточнено: должна ли она сбрасывать уже собранную Rush-колоду при
  выходе из deckBuilderModal, или сохранять черновик.
- [ ] **Красивая модалка подбора карт** — автор опишет дизайн подробнее непосредственно перед
  реализацией. Возможно пересекается с "Апгрейд дизайна окна декбилдера" в разделе "На
  подумать" ниже — уточнить на месте, один это запрос или два разных экрана (deckPickerModal
  vs deckBuilderModal).
...
- [ ] Дизайн оверлея "выбрать цель" (`#targetPromptOverlay`, styles.css/index.html) — сейчас
  надпись просто лежит на пульсирующем фоне поверх зоны руки; автор хочет добавить что-то
  вроде рамки/подложки под текст, чтобы лучше читалась поверх карт. Конкретики пока нет —
  на паузе, автор подумает.
...
## Sound checklist — от Муры, файлы на руках (записано 2026-07-06, обновлено 2026-07-07)

_Файлы уже у автора, подключение — в приоритете на завтра (см. Backlog — Code выше).
Отмечать по мере фактического подключения каждого звука в коде._
