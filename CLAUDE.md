...
- **Gates** (the body) — maps to the archetype/gtype grouping. A future task: show each
  ...
  **Author calls travelers by their Gate name in conversation** (e.g. “Orbiton”, “Merchird”) —
  ...
  |---------|------------|--------------------------------|------------------------------------|
  |Szarg    |`szg`       |`vanguard` (+regen/fear/burn)   |Squad: +1 maxHP.                    |
  |Orbiton  |`orb`       |`heal:1` (+vanguard/fear/burn)  |Active: Heal 1 HP. Squad: Heal 2.   |
  |Dreegan  |`drg`       |`provoke` (+regen/fear/vanguard)|Squad: +1 Armor.                    |
  |Umbasir  |`umb`       |`bolt:1` (+regen/fear/vanguard/untamed) |Active: Bolt 1 dmg (точечный магический урон, не AOE). Squad: Bolt 2 dmg. |

  2026-07-11, ещё позже та же сессия: Umbasir перестал быть дублем Orbiton. Новый тег
  **`bolt:N`** — активка на существе (как heal:1/aoe:1), но точечный МАГИЧЕСКИЙ урон по
  одному врагу (bypassArmor, блокируется Ward) — по сути SHARD, только не на артефакте,
  а на существе, со своими функциями `doUmbBolt()`/`doBoltTarget()` в `game.js` (SHARD
  и Bolt архитектурно независимы, как Umbasir/Vardan для AOE раньше). Как и у Shard — **+1
  урона, если цель Feared**. Squad-бонус Umbasir теперь `param:'bolt', val:2` (было `heal`) —
  та же механика замены итогового значения, что и раньше у AOE/Heal squad-бонусов.
  |Merchird |`mch`       |`pierce` (+rage/regen/burn)     |Squad: +1 ATK.                      |
  |Xuiqtr   |`xui`       |`regen` (+fear/burn/untamed)    |Squad: Regen 2.                     |

  2026-07-11, поздний пересмотр той же сессии: откатили Xuiqtr обратно с `provoke` на `regen`
  (было слишком много "танков" и путаницы) и убрали плату 2 эссенции за AOE-активку (тоже
  откат). Umbasir лишился `aoe:1` целиком — стал вторым Orbiton (`heal:1` + Squad Heal 2),
  осознанно оставлен дублем по решению автора, а не как временная заглушка. Итоговая
  структура шести Врат: **2 танка** (Dreegan — форсирует атаки на себя; Xuiqtr — "танк без
  провокации", живёт за счёт регена, но не контролирует, кого бьют), **2 атакующих**
  (Szarg — темп/vanguard; Merchird — пробивной/pierce), **2 хилера** (Orbiton и Umbasir —
  полные дубли друг друга, отличаются только конкретной HP/ATK-кривой по цене). AOE как
  архетипная механика Врат теперь не существует вообще — если понадобится, может вернуться
  через спеллы/артефакты/уников, не через Gate-тег.
...
### Essence pricing shop (v2) — радикальный пересмотр экономики, 2026-07-11

v1 (0.2 налог + 0.2 HP + 0.4 ATK, без потолка эссенции) дала на практике три реальные боли:
безлимитная эссенция → к позднему ходу вся рука разом выкладывается стеной; ATK вдвое дороже
HP → дешёвые тела массово жались в HP1 (умирают от любого чиха); AOE как бесплатная
многоразовая кнопка на дешёвом теле выкашивала именно те массово-хрупкие тела одним нажатием
каждый ход. v2 чинит все три причины разом, не только цифры на картах.

**1. Потолок эссенции — `ESS_CAP=10`** (`game.js`). essMax больше не растёт бесконечно —
капается на 10 во ВСЕХ трёх местах роста (сжигание карты, обычный прирост за ход, эффект
`ess_max` с Мира/Артефакта). `cur.ess` в рамках одного хода всё ещё может временно превышать
essMax (сжигание/`ess_add` — как и было), капается именно многоходовый потолок, не разовый
всплеск.

**2. AOE — больше не бесплатная кнопка.** `doUmbAsir()`/`doVardan()` в `game.js` теперь
списывают **2 эссенции** за каждую активацию (константа `AOE_ACTIVATION_COST`), поверх
обычной проверки `exhausted`. Если эссенции не хватает — активка просто не срабатывает
(лог-подсказка, ход не ломается). ИИ (`ai.js`) пока не проверяет запас эссенции ЗАРАНЕЕ перед
попыткой — не критично (просто иногда будет пытаться и получать отказ), но стоит доделать
отдельно.

**3. Цена "тела" — HP и ATK теперь РАВНЫ:**
- Налог за тело: **0.2 эссенции** (не менялся)
- HP: **0.3 эссенции/очко** (было 0.2)
- ATK: **0.3 эссенции/очко** (было 0.4 — именно перекос 2х толкал карты в HP1)
- Полностью убрана привязка "атакующим можно проседать в HP1" — теперь **глобальный пол
  HP=2 для ВООБЩЕ ВСЕХ рядовых travelers**, без исключений по архетипу.
- Потолок HP=5 у рядовых сохранён (Дриган/Xuiqtr/Orbiton/Umbasir) — излишек бюджета уходит
  в ATK, как и в v1.

**Цена тегов — пересмотрена, условные теги дешевле безусловных:**

| Тег | Цена | Комментарий |
|---|---|---|
| `gtype:xxx` | 0 | сама категория, ничего не делает |
| `provoke` | 1.5 | безусловный контроль всего боя |
| `rage` (с потолком) | 1.5 | |
| `aoe:1` | **1.0** (было 1.25) | активка теперь ещё и платная по эссенции (см. п.2) — тег может стоить дешевле |
| `vanguard` | 1.0 | безусловно полезен всегда |
| `heal:1` (активка) | 1.0 | |
| `burn` | 1.0 | |
| `incarnation:4` (фикс. для рядовых) | 1.0 | |
| `fear` | 0.75 | |
| `untamed` | 0.75 | |
| `regen` | 0.75 | |
| `pierce` | **0.5** (было 1.0) | условная способность — простаивает, если у соперника нет Provoke/Bushido, не должна стоить как безусловный тег |

**Базовый тег и цена-старт по вратам** (не изменилось от v1, кроме пола HP — теперь везде 2):

| Gate | Базовый тег | Cost-старт | Перекос статов |
|---|---|---|---|
| Szarg | `vanguard` | 2 | ATK, пол HP=2 |
| Merchird | `pierce` | 2 | ATK, пол HP=2 |
| Dreegan | `provoke` | 3 | HP (потолок 5) |
| Xuiqtr | `provoke` | 3 | HP (потолок 5) |
| Orbiton | `heal:1` | 2 | Минимум по обеим, потолок HP 5 |
| Umbasir | `aoe:1` | 3 | Минимум по обеим, потолок HP 5 |

Правило доп.трейтов не изменилось: **+1 к итоговой цене за каждый доп.тег** сверх базового
гейт-тега, остаток бюджета конвертируется в HP/ATK согласно перекосу гейта.

**Уникальные 1/1 — тоже пересчитаны в этой волне** (впервые формализована их цена):
- Своя таблица тегов: `draw:1`/`aura:*:1`/`draw_attack:1` = 1.5 (сильные постоянные движки),
  `bushido`/`ward` = 2.0 (самые мощные индивидуальные эффекты в игре), `on_play_creature:1`/
  `on_any_death_base:1`/`raise:1` = 1.0, `invisible` = 1.0, `pierce` = 0.5 (та же скидка, что
  и у рядовых — условность не зависит от редкости карты).
- `armor:N` у уников считается СТАТОМ, не тегом — по той же цене 0.3/очко, что и HP/ATK.
- `incarnation:X` у уников — цена по формуле **2.0 − 0.25×X** (быстрее возврат — дороже;
  X=3 → 1.25, X=4 → 1.0 как у рядовых, X=2 → 1.5).
- Потолки статов ослаблены относительно рядовых (уники имеют право быть круче): **ATK≤5,
  HP≤8**, остаток при превышении перетекает в другой стат.
- Разово подмечено: NABUNAGI (`bushido`+`ward`=4.0 эссенции тегов) при cost5 почти не
  оставляет бюджета на статы (HP2/ATK1) — с Бронёй это ещё жить может, но стоит держать в
  уме как кандидата на будущий пересчёт (либо поднять cost, либо чуть снизить цену
  `bushido`/`ward` по отдельности).

**v7 — базовые линии заданы автором напрямую (не выведены из формулы), явные цены Mood/World
трейтов в долях эссенции, 2026-07-11, финал сессии.**

**Базовая линия по Вратам (0 доп.тегов):**

| Gate | Тег | Cost | HP/ATK | Squad |
|---|---|---|---|---|
| Szarg | — (без тега вообще, чисто статы) | 1 | 1/2 | +1 maxHP |
| Merchird | `pierce` | 2 | 2/2 | +1 ATK |
| Dreegan | `provoke` | 2 | 3/1 | +1 Armor |
| Xuiqtr | `provoke` | 2 | 2/2 | +1 ATK |
| Umbasir | `bolt:1` | 1 | 1/1 | Bolt →2 |
| Orbiton | `heal:1` | 1 | 1/1 | Heal →2 |

Xuiqtr и Dreegan оба несут `provoke` (два разных танка, отличаются Squad-бонусом — Armor vs
ATK), Szarg **вообще без гейт-тега** — чисто статы, самый дешёвый архетип в игре. Xuiqtr
делит Squad-бонус (+1 ATK) с Merchird.

**Правило доп.тегов (1 бонус-трейт = +1 к cost, дальше зависит от цены самого трейта):**
- Трейт ценой **0.33** (дешёвый) → cost+1, и **+1 HP И +1 ATK** (сдачи хватает на оба стата)
- Трейт ценой **0.66** (дорогой) → cost+1, но **только +1 к профильному стату** (HP у
  Dreegan/Umbasir/Orbiton, ATK у Szarg/Merchird/Xuiqtr) — сдачи
  хватает только на один
- 2 доп.тега — правило применяется к каждому НЕЗАВИСИМО и складывается (не отдельная
  "ультра-редкая" надбавка, как было в v6 — эта версия проще: просто сумма по каждому тегу)

**Цены конкретных трейтов (зафиксировано автором, доли эссенции):**

| Trait-слот | Значение | Тег | Цена |
|---|---|---|---|
| Mood: Сердце | Реген | `regen` | 0.33 (дешёвый) |
| Mood: Флейм | Поджог | `burn` | 0.66 (дорогой) |
| Mood: Череп | Страх | `fear` | 0.66 (дорогой) |
| Mood: Аниме (все 4 цвета) | Неукротимость | `untamed` | 0.33 (дешёвый) |
| Mood: Свеча | Карта при ударе | `draw_attack:1` | 0.66 (дорогой) |
| Mood: Солана | Ward | `ward` | 0.66 (дорогой) |
| World: Эншент | Инкарнация | `incarnation:4` | 0.33 (дешёвый) |
| World: Блад | Рейдж | `rage` | 0.66 (дорогой) — **обязателен потолок накопления**, см. ниже |
| World: Нет | Инвиз | `invisible` | 0.66 (дорогой) |
| World: Пески | Авангард | `vanguard` (теперь ТОЛЬКО как бонус-трейт, не гейт-тег Szarg) | 0.66 (дорогой) |
| World: Долина | Добор в начале хода | `draw:1` (нужна доработка кода — см. ниже) | 0.66 (дорогой) |

**Важное предупреждение, не решено кодом:** "Долина → добор в начале хода" на РЯДОВОМ
travelers сейчас не заведено — `getAbilities()` (`abilities.js`) даёт тегу `draw` timing
`on_attack` для обычных существ (только `on_turn` для world/artifact/unique). Чтобы этот
конкретный трейт работал буквально "в начале хода", а не "при атаке", нужна отдельная правка
диспетчера (например: рядовой + `draw` → тоже `on_turn`, отдельно от `on_attack`-версии, или
новый явный тег типа `draw_turn:N` вместо переиспользования `draw`). Пока НЕ реализовано —
это открытая задача, не забыть при следующей сессии.

**Trait-слоты без назначения (пока свободны, ждут решения):** Optical Dope, Схема (Schema),
Бамбук, Незабываемый, Забудь всё (Forget Everything), Розовые облака, Solana World (мир,
не путать с Mood: Солана выше).

**Механики "россыпью", ещё не привязанные ни к одному конкретному слоту** (из отдельного
списка автора этой же сессии): AOE при входе (`enter_aoe`, уже есть в коде — ABYSSWALKER
раньше его нёс), Вампиризм (не реализовано, есть в бэклоге), полный "on_own_death"-пакет
(дать карту + ранить базу врага + дать эссенцию + вылечить — сейчас у нас это раздельные
теги на разных Мирах/униках, не единый пакет), Трупоедство/`necrophage` (не реализовано,
есть в бэклоге).

**Уники/Миры/Артефакты — явно отложено автором на следующую сессию.** Список механик,
которые туда планируются (некоторые уже частично реализованы на конкретных картах — не
переизобретать с нуля, просто дособрать в цельные колодообразующие движки): ауры (HP/ATK/
Armor — уже есть), Некромантия/`raise` (уже есть на PHLEGMOR), Бусидо (уже есть на
NABUNAGI), AOE по кнопке на Мире/Артефакте, триггер на смерть (своей/чужой карты) → хил
базы + добор, триггер на розыгрыш карты → хил базы (уже есть FAERON), триггер в начале хода
→ добор + доп.эссенция + хил всем на 1 (частично уже есть по отдельности на VALLEY/THE BOOK/
FOUNTAIN — не собрано в одну карту), активка "Жертва" (уже есть ALTAR).

**UI/иконки, сделано в эту же сессию:** `incarnation` получил тег-иконку `ico_incarn.png`
(добавлена во все 5 копий TAG_ICONS/DB_TAG_ICONS + TAG_TOOLTIPS в `ui.js`) — текст
"Incarnation N." убран из `ab` везде (включая PHLEGMOR), теперь только иконка+тултип при
наведении. Bolt-кнопка (Umbasir) теперь тоже переключается на красный крестик-отмену
во время выбора цели — тот же паттерн, что уже был у кнопки Heal (`isCancellingBolt`,
`render.js`).

**Открытая задача, НЕ сделано в этот раз** (нужно уточнение автора): фон счётчика кладбища
`armor_bg.png` (2:1, вертикально) с иконкой `ico_incarn` — формулировка неоднозначна (новый
отдельный индикатор рядом со счётчиком кладбища, показывающий число карт с активным
таймером Инкарнации? или замена фона самого счётчика?) — не стал угадывать, уточнить в
следующей сессии.

**Обновление той же сессии (позже):** уточнено — `armor_bg.png` это просто ЗАМЕНА фона
счётчика кладбища (не новый отдельный индикатор). Сделано: `#teaGraveBadge`/`#jeetGraveBadge`
теперь используют `armor_bg.png` вместо общего `bg_statbar_hp.png`/`bg_counters_jeet.png` —
счётчик колоды (Deck) фон не менял, только Grave.

**HP базы поднят с 20 до 30** (тест) — `state.js` (`initP()`) + `hpTier()` пересчитана
пропорционально (тиры визуала статус-бара теперь на 24/18/12/6, а не 20/15/10/5).

**Xuiqtr — регенерация полностью убрана со всех 8 карт**, остался только базовый `provoke`
(+ у части карт другие бонус-теги: untamed/fear/burn) — реген как отдельная фишка Xuiqtr
больше не существует вообще, только Squad ATK.

**Новые ценовые открытия из точечных правок этой сессии** (получены реверс-инжинирингом
конкретных cost/hp/atk, которые задал автор напрямую — не выведены из старой формулы):
- **`bolt:1` теперь стоит 0.66** (было 0.33) — вскрылось через #52/#6/#54 (Umbasir): при
  cost1, hp1, **atk0** (не atk1!) сходится ровно на 0.66 для тега. Это меняет саму базовую
  линию Umbasir: **база теперь `bolt:1`, cost1 → 1/0**, а не 1/1 — сила карты целиком в
  кнопке, тело почти чисто номинальное (0 ATK — не может атаковать в бою вообще).
- **Ауры (`aura:*:1`, уники) — 2.0** (было 1.5-2.0 в разных версиях, теперь явно зафиксировано
  автором как 2.0 — TUBORG/ASLEX/ABYSSWALKER пересчитаны под это)
- **Некромантия (`raise:1`, уники) — 2.0** (было 1.5)
- **Броня (`armor:N`, за очко) — 0.66** — то есть ДОРОЖЕ HP/ATK по отдельности (0.33 каждый),
  Броня как стат ценится в 2 раза выше обычного очка тела — логично, она эффективнее блокирует
  урон, чем сырой HP.

**Обновление той же сессии, ещё позже:**

- **Фон счётчика кладбища** — откачен обратно (не трогаем общий счётчик карт). Правильно
  понятая задача: `armor_bg.png` идёт на уже существующий (с более ранней сессии)
  `.card-incarn-badge` — вертикальный бейдж поверх карты в модалке кладбища у карт с
  активным таймером Инкарнации (иконка сверху, число ходов снизу). Раньше был цветной
  плейсхолдер (`rgba(20,10,30,0.75)`), теперь — `armor_bg.png`, повёрнутый на 90° через
  `::before`-псевдоэлемент (сам ассет горизонтальный 2:1, бокс вертикальный ~1:2.18) —
  swap ширины/высоты ДО поворота, чтобы после rotate ровно лёг в бокс. Иконка внутри тоже
  заменена с эмодзи-плейсхолдера на `ico_incarn.png`.
- **Тултип Инкарнации теперь показывает конкретное число X** — раньше `TAG_TOOLTIPS`
  был статичным текстом на весь тег, без параметра конкретной карты. Теперь `data-tagval`
  прокидывается в разметку иконки (`render.js`, обе копии — `mkEl`/`mkSmallEl`), и
  `_tooltipDataFor()` (`ui.js`) подставляет его в заголовок ("Incarnation 4" вместо просто
  "Incarnation"). Пока сделано только для игрового поля — каталог/дек-билдер тем же тегом
  подсказки не показывают вообще, там доработка не требовалась.
- **SCHEME / BLACK MAGIC** → cost 0 (были 1).
- **ARCHIVE** → cost 3 (было 2), и бафф теперь **"до конца боя"**, а не "до конца хода" —
  раньше `tempAtkBonus` сбрасывался сразу в конце хода того же игрока (`endTurn()`,
  до того как ход вообще передавался сопернику) — теперь сброс перенесён в начало
  СЛЕДУЮЩЕГО хода ВЛАДЕЛЬЦА (тот же блок, что армор-рефилл) — баф переживает весь ход
  соперника целиком, полезен и как оборонительный трюк (усиленная контратака), не только
  атакующий.

**Новые ценовые подтверждения из точечных правок (реверс-инжиниринг):**
- `bushido` = **2.0** (через NABUNAGI: armor:2→armor:1, статы не изменились)
- `on_play_creature:1` = **2.0** (через FAERON)
- `on_any_death_base:1` = **2.0** (через REAPER, статы не изменились — просто подтверждение)
- Аура (`aura:*:1`) = **2.0**, подтверждено трижды независимо (ASLEX/TUBORG/ABYSSWALKER)
- `raise:1` (Некромантия) = **2.0** (через PHLEGMOR)
- **Формула цены Инкарнации на уников** (переменный X, вариативность только у уников):
  **цена ≈ 2.64 / X** — то есть обратно пропорционально скорости возврата. Проверка:
  X=4 → 0.66 (совпадает с ценой у рядовых), X=2 (PHLEGMOR) → 1.32≈1.33 (вдвое быстрее —
  вдвое дороже). Чистая, логичная зависимость — чем быстрее воскресает, тем дороже.
- TEANTIST получил `ward` дополнительно к `draw:1` — тестовая проверка "Долина" (добор в
  начале хода) на цене 1.33, поверх уже посчитанного `draw:1` на уникальной карте.

**Ещё не сделано / известная техническая дыра (не забыть):** трейт "Долина → добор в
начале хода" на РЯДОВЫХ travelers по-прежнему не заведён технически (диспетчер `draw` в
`abilities.js` даёт обычным существам только `on_attack` таймингом, `on_turn` — только
world/artifact/unique). У TEANTIST это работает только потому, что он уникальный
(`card.unique` уже даёт `on_turn` в диспетчере). Нужна отдельная правка, если этот трейт
когда-нибудь понадобится на рядовом травелере.

**Уники/Миры/Артефакты — пересчёт отложен на следующую сессию** (уже сказано автором).
Отдельно отмечено: у Миров/Артефактов должна появиться "надбавка за неуязвимость" —
некоторые их эффекты (постоянная аура на ВСЮ команду, а не одну карту) по сути неубиваемы
без прямого уничтожения самого Мира/Артефакта, и это стоит учитывать как отдельный
множитель цены, не сводить к сумме тех же тегов, что на обычных картах.
каждый раз), 2026-07-11, финал сессии.** Вместо жёсткого "1 тег=+1цена, 2 тега=+1цена+1стат"
— явные ОЧКИ редкости, которые тратятся по дизайнерскому решению на цену ИЛИ на стат:

- **Частая** (0 доп.тегов сверх Gate-тега) — 0 очков, чистая база
- **Редкая** (1 доп.тег) — **1 очко**
- **Ультра-редкая** (2 доп.тега) — **3 очка** (не 2 — стакинг трейтов специально ценится
  больше суммы частей)

**Базовая линия по Вратам (используется в п.1 ниже "знать Gate"):**

| Gate | Тег | Старт (0 доп.тегов) |
|---|---|---|
| Szarg | `vanguard` | cost2 → 2/2 |
| Merchird | `pierce` | cost2 → 2/2 |
| Dreegan | `provoke` | cost3 → 4/1 |
| Xuiqtr | `regen` | cost3 → 4/1 |
| Umbasir | `bolt:1` | cost1 → 1/1 |
| Orbiton | `heal:1` | cost2 → 2/1 |

Каждое очко идёт либо в **+1 к cost**, либо в **+1 к профильному стату** (HP у Dreegan/
Xuiqtr/Umbasir/Orbiton, ATK у Szarg/Merchird) — решает не формула, а то, СИЛЬНЫЙ тег или
нишевый:

| Категория | Теги | Куда уходит очко |
|---|---|---|
| Сильный (безусловная польза/билд-эраунд) | `burn`, `incarnation:4`, `rage`, `vanguard` (когда он ДОП.тег не на своих Врата) | +1 cost |
| Слабый/нишевый (ситуативная польза) | `fear`, `untamed`, `regen` | +1 к профильному стату |

Для ультра-редких (2 тега, 3 очка): считаем **S** = сколько из двух тегов сильные (0/1/2).
`price_bonus = S+1`, `stat_bonus = 2−S`. Т.е. два слабых тега → 1 очко в цену + 2 в статы;
два сильных → 3 очка в цену, статы не растут; один+один → 2 в цену + 1 в статы.

**Это и есть прямой ответ на вопрос "может ли дешёвый тег на редкой карте компенсироваться
статами"** — да, именно так очко тратится, если тег слабый: карта не становится ловушкой
(переплата без пользы), она либо дороже за сильный эффект, либо крепче телом за слабый.

**Как считать ЛЮБУЮ новую карту из будущих 1100** — нужно знать только три вещи:
1. Её Gate (даёт базовую линию cost→hp/atk, уже зафиксировано для всех шести)
2. Её редкость (даёт число очков — 0/1/3, при необходимости можно завести Legendary=5-6)
3. Разовое решение по каждому НОВОМУ тегу — сильный он или нишевый (после первого решения
   тег входит в таблицу выше и дальше применяется автоматически)

Applied 2026-07-11 ко всем 48 текущим рядовым (см. `data.js`) — единственное ручное
суждение потребовалось один раз (классификация ~7 существующих доп.тегов на сильные/слабые),
дальше всё считается механически.

---
**Ниже — архив предыдущих итераций той же сессии (v2→v3→v4), оставлен для истории, НЕ
актуальная логика.** Финальное правило — v5 выше.

**v3 (устарело) — теги оказались переоценены даже после v2, шкала сужена до 0.3 (самый
условный/слабый) —
1.0 (самый мощный в игре), 2026-07-11, поздний пересчёт той же сессии:**

| Тег | Цена v2 | Цена v3 (текущая) |
|---|---|---|
| `pierce` | 0.5 | **0.3** |
| `fear` / `untamed` / `regen` | 0.75 | **0.4** |
| `vanguard` / `heal:1` / `burn` / `incarnation:4` / `aoe:1` | 1.0 | **0.6** |
| `provoke` / `rage` | 1.5 | **0.8** |
| (уники) `draw:1` / `aura:*` / `draw_attack:1` | 1.5 | **0.8** |
| (уники) `on_play_creature:1` / `on_any_death_base:1` / `raise:1` / `invisible` | 1.0 | **0.6** |
| (уники) `bushido` / `ward` | 2.0 | **1.0** |
| `incarnation:X` у уников | 2.0−0.25X | **0.6+0.1×(4−X)** (incarnation:4=0.6, совпадает с рядовыми) |

Относительный порядок силы тегов не менялся — просто весь диапазон сжат в [0.3, 1.0].
Из-за этого бюджет на статы у рядовых сильно вырос — пришлось ввести **потолок ATK=4**
(симметрично уже существовавшему потолку HP=5) для рядовых, иначе атакующие улетали в
ATK5-6 на cost2-3. Излишек сверх обоих потолков (редкий случай) просто не выражается в
статах — то есть карта чуть выгоднее номинальной цены, не критично.

**v4 (устарело, см. v5 выше) — калибровка по фидбеку автора "как должно ощущаться", 2026-07-11.**
Автор дал конкретные якоря (какая карта на какой цене как должна выглядеть) — цены v3 не
подгонялись под формулу вслепую, а решались как уравнения под эти якоря. Совпадения почти
все — точные:

| Архетип | Якорь автора | Решённая цена базового тега |
|---|---|---|
| Szarg (`vanguard`) | cost2 → 2/2 | **0.6** |
| Merchird (`pierce`) | cost2 → 2/2 | **0.6** (пересмотрено вверх с 0.3 — автор явно хочет Мехирда наравне со Сзаргом) |
| Orbiton (`heal:1`) | cost2 → 3/1 | **0.6** |
| Umbasir (`aoe:1`) | cost2 → 2/1 (Umbasir теперь **тоже стартует с cost2**, был cost3) | **0.9** |
| Dreegan/Xuiqtr (`provoke`) | cost3 → 4/1 | **1.3** |

Все пять базовых тегов сошлись РОВНО в целевые числа автора при этих ценах — совпадение
точное, не округление. Вторичные (бонусные) теги на рядовых: `fear`/`untamed`/`regen`=0.4,
`burn`/`incarnation:4`=0.5, `rage`=0.9 (держим на уровне aoe/provoke — растёт без потолка).

**Уники — двухуровневая цена, автор прямо это сформулировал**: "ауры и воскрешение и
остальные УНИКАЛЬНЫЕ теги — отдельная цена, обычные теги можно со скидкой":

| Тег | Цена (эксклюзив у уников) |
|---|---|
| `draw:1`, `on_any_death_base:1`, `aura:maxhp/atk/armor:1`, `draw_attack:1` | 2.0 |
| `on_play_creature:1`, `raise:1`, `invisible` | 1.5 |
| `bushido`, `ward` | 2.5 |
| `incarnation:X` (уники) | 1.5 + 0.25×(4−X) |
| Общие с рядовыми теги (`fear`/`burn`/`untamed`/`pierce`/`vanguard`/`provoke`) | цена рядовых, БЕЗ наценки |

HP/ATK у всех 10 уников оставлены родными (как были исторически, до любых правок в этой
сессии) — формула считала ТОЛЬКО `cost`. Совпало почти всё: TEANTIST cost4 ✓, FAERON cost4 ✓,
REAPER cost5 ✓, SEEKER cost4 ✓, PHLEGMOR cost6 (автор целился в 5-6) ✓.

**Одно жёсткое исключение — NABUNAGI.** `bushido`(2.5)+`ward`(2.5) = 5.0 эссенции чистыми
тегами — формула требует cost8, что ломает потолок (никто больше не дороже 6). Автор решил
не трогать цену тегов ради одной карты — **cost зафиксирован вручную на 6**, статы (6/2)
остались как есть. Если позже добавятся другие bushido+ward карты — вопрос цены этой пары
тегов стоит поднять снова, а не продолжать делать поштучные исключения.

**Потолки статов у рядовых, действующие в этой версии**: HP-пол 2 (везде, без исключений
по архетипу), HP-потолок 5 (Dreegan/Xuiqtr/Orbiton/Umbasir), ATK-потолок 4 (везде). Излишек
бюджета сверх обоих потолков одновременно (редкий случай) просто не выражается в статах.
...
- **Идеи из Astral Masters (Владыки Астрала)** — в отличие от Берсерка, у этой игры нет
  формализованных именованных ключевых слов, способности просто текстом на карте — ниже
  уже переведено в наш тег-формат по паттернам, которые повторяются на многих картах:
  - **`atk_eq_ess`** — ATK существа = текущая Эссенция владельца (динамический скейлер от
    ресурса, а не от статичного числа). Пример-первоисточник: Elf Mystic.
  - **`sweep`** — обычная атака (не активка!) бьёт ВСЕХ вражеских существ сразу, в отличие
    от нашего `aoe`, который требует кнопки/хода. Обычно на статах послабее в компенсацию.
    Пример: Forest Sprite ("Attacks all enemies").
  - **`fragile:N`** — получает +N к ЛЮБОМУ входящему урону (из любого источника). Балансировочный
    рычаг-даунсайд для дешёвых/сильных карт — не бафф, а осознанная уязвимость. Пример: Storm
    Drake ("All damage done to it is increased by 2").
  - **`on_spell_cast:N`** — при каждом сыгранном СВОЁМ заклинании наносит N урона сопернику
    (не бафф ATK, а прямой урон) — альтернативная реализация уже записанной у нас идеи
    "+X ATK за каждое заклинание" (Version 1.01, п. про новый тег) — можно держать обе версии
    рядом до финального решения, какая лучше ляжет. Пример: Goblin Shaman.
  - **`on_draw:N`** — триггерится на сам факт добора карты владельцем (не только "у карты есть
    draw", а реакция на чужой/свой добор вообще) — draw-matters пейофф. Пример: Faerie Peacekeeper.
  - **`on_play_gtype:N`** — при розыгрыше ЕЩЁ ОДНОЙ карты своего архетипа (`gtype`) наносит
    N урона сопернику. Естественно ложится на уже существующую систему архетипов (`gtype:szg/
    orb/drg/umb/mch/xui`) и Squad — ещё один тип пейоффа архетипной колоды, отдельный от порога
    Squad. Пример: Dwarven Crossbowmen ("whenever owner summons a dwarf...").
  - **`backlash:N`** — активка наносит урон врагу, но и N урона себе (риск/награда, платится
    HP, а не эссенцией). Пример: Merfolk Hunter ("Deals 10 dmg to target + 6 dmg to self").
  - **Мультиудар от порога эссенции** (вариант `double_strike` из бэклога выше, но условный:
    "если эссенции ≥ N — атакует несколько раз за ход", а не безусловно) — ближе к
    "экономическому" архетипу, которого пока не хватает Jeet (см. `ess_steal` выше).
  - Позиционные механики Astral Masters (проверки "что стоит в слоте напротив") сознательно
    НЕ переносим — у нас нет позиционирования существ на поле вообще, как и с сеткой Берсерка.

- **Идеи из Hearthstone** — золотая середина между Берсерком (чистые именованные кейворды)
  и Astral Masters (вообще без кейвордов), многое уже близко к тому, что у нас есть:
  - **`shield`** (Divine Shield) — поглощает РОВНО ОДИН ближайший удар целиком, от ЛЮБОГО
    источника (физика ИЛИ магия), затем тег снимается. Третий слой защиты рядом с Бронёй
    (многоразовый пул, только физика) и Ward (безлимитный, только магия) — закрывает нишу,
    которую не закрывают они.
  - **`poisonous`** (Poisonous) — execute-механика, которой у нас вообще нет: ЛЮБОЙ урон от
    этого существа в бою убивает цель мгновенно, вне зависимости от цифр ATK/HP. Билд-эраунд
    потенциал ("маленький, но ядовитый убивает гиганта").
  - **`deathrattle:effect:val`** (Deathrattle) — генерализация того, что у нас уже есть
    кусками (`on_own_death` на мире, `on_any_death_base`, `on_kill_base` — все точечные,
    завязаны на конкретные карты). Deathrattle — универсальная инфраструктура "когда УМИРАЕТ
    ИМЕННО ЭТА карта — сделай X (урон/добор/бафф союзнику)", на самой карте, не на постороннем
    триггере.
  - **`enrage:atk:N`** (Enrage) — существо получает бонус, ПОКА оно повреждено (не на полном
    HP), теряет бонус при лечении до полного. Риск/награда от собственного текущего HP.
  - **`overload:N`** (Overload) — карта сильнее, но следующий ход essMax владельца на N меньше.
    Тот же принцип риск/награда, что и `backlash:N` из Astral Masters, но платится темпом
    (эссенцией), а не HP — вторая "валюта расплаты" рядом с первой.
  - **`reborn`** (Reborn) — воскрешает СЕБЯ МГНОВЕННО в момент первой смерти (на 1 HP), без
    задержки по ходам, один раз. Быстрый родственник нашей `incarnation` (та воскрешает через
    X ходов) — разные ниши, не дублирует.
  - **`spell_silence`** (Silence) — обобщение уже записанного выше `taunt_break` (который
    снимает только Provoke/Bushido): снимает С ЦЕЛИ ВООБЩЕ ВСЕ теги/ауры/дебаффы. Можно
    держать оба — `taunt_break` как узкий дешёвый контрплей, `spell_silence` как дорогой
    универсальный ластик.
  - **`twinspell`** (Twinspell) — после розыгрыша в руку возвращается вторая (обычно более
    слабая) копия того же спелла, один раз.
  - **`frozen`** (Freeze) — отдельный дебафф от `feared`: Fear блокирует контратаку, Frozen —
    цель пропускает СВОЙ следующий ход атаки, но контратаковать в ответ всё ещё может. Разные
    ниши, оба имеют смысл рядом.
  - **`world_spelldmg:N`** (Spell Damage) — мир, усиливающий числа магического урона (Shard/
    AOE/точечный спелл) у ВСЕХ карт владельца. Ложится на уже готовую world-инфраструктуру,
    по образцу `world_armor`/`world_maxhp` — не с нуля.
  - Не берём: Magnetic (позиционное "приклеивание" карт друг к другу — нет инфраструктуры),
    Discover (нужен отдельный UI "выбери 1 из 3 показанных карт" — тяжело, не тег),
    Adapt/Corrupt/Outcast (завязаны на позицию карты в руке/классовую специфику Hearthstone).

- **Идеи из Magic: The Gathering** — самая большая база (194 кейворда в игре), берём не всё,
  а то, что реально ново и не дублирует уже отложенное. Дублирующее НЕ повторяем отдельным
  пунктом — уже покрыто: Deathtouch≈`poisonous`, Lifelink≈`vampiric`, Vigilance≈`untamed`.
  Flying/Reach/Menace не переносятся вообще — завязаны на систему блокеров, которой у нас нет
  (Provoke — единственное, что заставляет отвечать на удар, но это не то же самое, что выбор
  блокера). **ВАЖНО**: у MTG есть свой кейворд "Ward" — это СОВСЕМ другое (контрит спелл
  оппонента, если тот не доплатит X, налог на таргетинг) — не путать с уже занятым у нас
  тегом `ward` (магический иммунитет, ближе к их Hexproof/Protection). Ниже используем другое
  имя, чтобы не столкнуться.
  - **`first_strike`** — см. отдельное обсуждение ниже (ломает симметрию одновременного
    обмена ударами, которая сейчас работает всегда).
  - **`trample`** (адаптированный под нас — у нас нет блокеров, зато есть базы) — если атака
    существа убивает цель, излишек урона сверх её HP уходит в базу противника.
  - **`defender`** — не может атаковать вообще. Даунсайд-рычаг для дешёвых жирных стен.
  - **`indestructible`** — не умирает от урона (HP не может дойти до летального от дамага),
    но всё ещё уязвим к bounce/sacrifice/войду. Другая ниша, чем Броня/Ward/Shield — те про
    снижение урона, это про полный иммунитет именно к "смерти от дамага".
  - **`hexproof`** — не может быть ВЫБРАН целью точечного спелла/Shard соперника вообще
    (в отличие от Ward — тот даёт спеллу отработать, но гасит урон; AOE Hexproof не спасает,
    т.к. AOE не таргетит конкретную карту).
  - **`exalted`** — бонус ATK/HP существу, если в этот ход атаковало ТОЛЬКО оно одно.
    Тактический выбор "бить всем скопом или придержать ради бонуса одному".
  - **`annihilator:N`** — если существо пробивает атаку НАПРЯМУЮ по базе (не по существу),
    противник жертвует N своих существ. Мощный финишер для дорогих легендарок.
  - **Persist/Undying** — обогащают уже собранную "семью воскрешений" (`raise`/`revive`/
    `incarnation`/`reborn`/`necrophage`): воскресает, но с ПОСТОЯННЫМ модификатором статов
    при возврате (слабее либо сильнее) — гранулярность, которой пока не хватает.
  - **`flashback:N`** — спелл на кладбище можно разыграть ещё раз за N эссенции, один раз.
    Продолжает тему интереса к своему кладбищу (рядом с `necrophage`/`grave_scale`).
  - **`convoke`** — часть стоимости карты можно оплатить не эссенцией, а истощением
    (`exhausted`) своих существ вместо траты ресурса.
  - **`kicker:N:эффект`** — необязательная доплата к спеллу за усиленный эффект — не
    отдельная механика, а модульный паттерн для будущих спеллов.

- **Идеи по сжиганию карт (burn-as-resource)** — сейчас сжигание жёстко ограничено 1 картой
  за ход (`cur.burned`, см. `doBurnCard()` в game.js). Две карты-компаньона под эту тему:
  - **`unlimited_burn`** (мир или артефакт) — карта на поле снимает лимит "1 раз за ход":
    пока она в игре, `cur.burned` не выставляется/не проверяется — можно сжигать сколько
    угодно карт за один ход. Чисто про снятие текущего ограничения, without new burn effects.
  - **`on_burn:N`** (скорее всего под 1/1) — при каждом сжигании ВЛАДЕЛЬЦЕМ карты (любой,
    не только этой) владелец добирает N карт. Вместе с `unlimited_burn` — потенциальный
    комбо-движок: сожги всю руку → пересобери её через добор, попутно получив кучу эссенции
    (сжигание уже даёт essMax+1/ess+1 за карту). Совсем незащищённый от абьюза комбо, если
    N=1 и сжигание безлимитно — сознательно нужно будет посмотреть на баланс отдельно, когда
    дойдём до реализации (не на брейншторм-этапе).
