...
- [x] **5 правок по фидбеку автора** — 2026-07-10.
  1. **Звук/анимация добора карты вне обычного хода (Hunger/Altar/spell-draw/Ryvlen
     on-attack)** — карты приходили в руку молча, без fly-анимации из колоды. Причина:
     `render()` обновлял видимость `SidebarBtns`/`BottomBar` (внутри которого физически лежит
     `deckPlaceholderT`/`J` — см. index.html) ПОСЛЕ вызовов `rZone()` для рук, а те внутри себя
     дёргают `_deckPlaceholderRect()`, которая проверяет `offsetParent` (видимость). Добор,
     случившийся вне стандартного "начало хода" момента, попадал на устаревшее состояние
     bottom bar с ПРОШЛОГО рендера — `_deckPlaceholderRect()` молча возвращала `null`, fly-
     анимация и её звук пропускались целиком. Переставил блок видимости `SidebarBtns`/
     `BottomBar` ВЫШЕ вызовов `rZone()` в `render()` — теперь к моменту вычисления rect бар
     уже в актуальном состоянии для текущего `G.turn`. Не трогал сами 4 источника добора
     (Hunger/Altar/spell-draw/Ryvlen) — они уже итак корректно пушат карты и вызывают
     `render()` один раз, чинить нужно было исключительно порядок внутри `render()`.
