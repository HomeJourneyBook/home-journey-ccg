...
```
HP  = cost
ATK = cost + 1      без исключений на любом cost
```
Живые числа: cost1 → 1/2, cost2 → 2/3, cost3 → 3/4, cost4 → 4/5.

> **История (2026-07-26):** раньше здесь было исключение "cost≥4 И ≥2 доп.тега → ATK=cost"
...
**cost5 Szarg — решение отложено (2026-07-26), не введён в data.js.** Причина — не баланс
...
> **История:** 2026-07-24 Mechird был поднят до `ATK=cost` (см. правку выше — версия
...
> оставить Szarg как единственного архетипа с ATK=cost+1 на верхнем крае.
>
...
- **Gates** (the body) — maps to the archetype/gtype grouping. A future task: show each
  ...
  **Author calls travelers by their Gate name in conversation** (e.g. “Orbiton”, “Merchird”) —
  ...
  |Orbiton  |`orb`       |`heal:1`                        |Squad: +2 Armor (было Heal, см. история 2026-07-26). |
  |Dreegan  |`drg`       |`provoke`                       |Squad: +1 Armor.                    |
  |Umbasir  |`umb`       |`bolt:1`                        |Squad: Bolt 2 dmg.                  |
  |Merchird |`mch`       |`pierce`                        |Squad: +1 ATK.                      |
  |Xuiqtr   |`xui`       |`intercept` (было `provoke` до 2026-07-17, см. Tag System ниже — таблица тут не была обновлена вовремя) |Squad: +1 ATK.                      |
