...
Ультраредкие (по 5 шт. в коллекции), названы автором 2026-07-27, тега/кода пока НЕТ:
- **Orange from FFF** → «Foxy Skill» — 50% шанс уклониться от получения урона.
...
- **To the Moon with DHD** → «Game of Market» (`market`, 2026-07-28) — при каждой
  атаке/Bolt 50/50: либо +2 бонус-урона по цели (или базе — прямой удар по базе тоже
  считается), либо −2 HP себе.
- **Winter from RGB** → «Frost Attack» — заморозка противника при атаке, механика будет
  расписана отдельно.
- **Nanas from SMB** → «Nana» (`nana`, ico_nana.png, 2026-07-29) — при каждой атаке/Bolt
  50/50: банан либо по случайному врагу (2 урона, физический, всегда игнорирует Ward),
  либо по случайному раненому союзнику/своей базе (2 хила), с фолбэком на дамаг-ветку
  если лечить некого/нечего.
- **DD's Signature** → «DD Cleave» (`dd`, ico_dd.png, 2026-07-29) — при обычной атаке
  (не Bolt) 1 физ. урон всем ОСТАЛЬНЫМ картам противника на поле, синхронно, без
  случайности.
