...
- **`GAME_VERSION` (`js/data.js`) НЕ поднимать без явного разрешения автора** — даже если
  ...
  версию.
- **Структура файла (актуализировано этой ревизией)**: документ теперь идёт в порядке —
  ...
  ПРЯМО СЕЙЧАС), (3) полная история правок сессия за сессией, (4) список художника (арт-долг),
  (5) backlog/незакрытые задачи, (6) план на маркетплейс/Version 2.0 — в самом конце,
  как самый дальний по времени раздел. Секция (2) — источник правды по текущему состоянию
  игры; секция (3) объясняет, ПОЧЕМУ всё стало так, как есть, но её цифры почти всегда УЖЕ
  устарели относительно секции (2) — при противоречии верить секции (2) и живому коду, не
  истории.
...
- Полная реструктуризация `CLAUDE.md` — вынесен единый актуальный срез архетипов/тегов/
  ...
  — ИИ не оценивает их при выборе карты для розыгрыша (см. Backlog).
