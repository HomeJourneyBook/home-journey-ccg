...
- [ ] 398 — Орбитон с 7 HP и 1 бронёй, "чот дохуя HP", пересмотреть баланс статов Орбитонов
...
- [ ] Для артефактов, ожидающих действие (активный, но ещё не использован) — сменить
...
- [ ] Добавить паддинги в текстовый бокс существ — текст сейчас наезжает на рамку карты.
- [ ] Иконка Intercept (`ico_intercept.png`) нигде не рендерится на самой карте (тултип/
  правила уже есть текстом — см. "Итог сессии 2026-07-17 (продолжение)" — но самого значка
  на карте не видно).
- [ ] TRAVELER #1015 (Dreegan, Jeet) — куда пропал Авангард? Был ли он вообще? Пересчитать
...
- [ ] Когда существо теряет Броню — добавить всплывающую циферку "-X Armor" (`showFloat()`),
  сейчас такого фидбека нет вообще (только когда броня добавляется/восполняется).
- [ ] Добавить иконки + тултипы для `stealth` ("Скрытность") и `thorns` ("Огненный Щит") —
  ...
  два — ещё нет.
- [ ] Solana Shield (`shield` тег) — сейчас единственный тег, у которого нет иконки-на-карте/
...
- [ ] Новый спелл — "поджечь все карты" (аналог `spell_fear_all`, но burn вместо fear),
...
- [ ] для скрытности и невидимости, помимо тегов делать еще карты полупрозрачными (у скрытности
...
- [ ] обьяснить ИИ, что нет смысла бить его картой 1/1 существо с броней 1, чтобы тупо получить ответку и умереть, если только у него нет других карт на поле, чтобы было резонно снять эту броню или идея зачем то умереть для тиггера on death
- [ ] обьяснить ИИ, что умбасиром лучше использовать болт для нанесения 1 урона, чем бить своей атакой 1 (если она не больше, там еще будет логично), чтобы не получать ответку
- [ ] Проверить Кситра -уставшего, он должен перенаправлять атаку на себя, но не давать контратаку
- [ ] в текстах карт уменьшить расстояние между слов
