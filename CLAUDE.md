...
- [x] **Modal "обвес"-футер** (`.modal-plated` in styles.css) — the bg_jest.png button plate is pulled out of the modal's flow and pinned absolutely, centered on the bottom frame bar, half overhanging ("лист жести, прибитый к рамке сверху"). Opt-in class because it requires `overflow:visible` (base `.modal` clips at rounded corners), so it must NOT go on internally-scrolling modals (deckBuilderModal). Applied to confirmModal + winModal; to plate any other non-scrolling modal, just add `modal-plated` next to `modal`. The pre-existing `.modal-decor` corner system (unused until now) remains available for further frame attachments — note it has the same `overflow` constraint.
