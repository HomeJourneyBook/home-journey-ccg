...
### Essence pricing shop — актуальная экономика (единая таблица, без истории правок)

Каждая карта считается по единой логике: тело (HP/ATK) + базовый тег Врат (для рядовых)
+ доп.теги (редкие трейты) сверху. Ниже — только то, что действует ПРЯМО СЕЙЧАС.

**Цена тела и базовых механик (одна цена везде, не зависит от типа карты):**

| Что | Цена |
|---|---|
| 1 HP | 0.33 |
| 1 ATK | 0.33 |
| 1 очко Брони сверх базовой (`armor:N`) | 0.66 |
| `provoke` | 0.66 |
| `pierce` | 0.66 |
| `bolt:1` | 0.66 |
| `heal:1` | 0.33 |

**Базовая линия рядовых по Вратам** (0 доп.тегов, тело+базовый тег уже включены выше):

| Gate | Тег | Cost | HP/ATK | Squad-бонус |
|---|---|---|---|---|
| Szarg | — (без тега, чистые статы) | 1 | 1/2 | +1 maxHP |
| Merchird | `pierce` | 2 | 2/2 | +1 ATK |
| Dreegan | `provoke` | 2 | 3/1 | +1 Armor |
| Xuiqtr | `provoke` | 2 | 2/2 | +1 ATK |
| Umbasir | `bolt:1` | 1 | 1/1 | Bolt +1 (2 итого) |
| Orbiton | `heal:1` | 1 | 1/1 | Heal +1 (2 итого) |

**Правило доп.тегов у рядовых** (редкие трейты — Mood/World, см. таблицы ниже): каждый
доп.тег добавляет **+1 к cost**. Дальше — по цене самого тега:
- Тег ценой **0.33** (дешёвый) → **+1 HP и +1 ATK** (сдачи хватает на оба стата)
- Тег ценой **0.66** (дорогой) → только **+1 к профильному стату** (HP у Dreegan/Umbasir/
  Orbiton, ATK у Szarg/Merchird/Xuiqtr)
- Доп.бафы НЕ могут давать активки, `provoke` или `pierce` — это эксклюзив самих Врат.

**Mood-трейты (рядовые):**

| Rarity | Trait | Тег | Цена |
|---|---|---|---|
| Rare | Аниме (все 4 цвета) | `untamed` | 0.33 |
| Rare | Сердце | `regen` | 0.33 |
| Legendary | Флейм | `burn` | 0.66 |
| Legendary | Череп | `fear` | 0.66 |
| Legendary | Свеча | `draw_attack:1` | 0.66 |
| Legendary | Солана | `ward` | 0.66 |

**World-трейты (рядовые):**

| Trait | Тег | Цена |
|---|---|---|
| Эншент | `incarnation:4` | 0.66 |
| Блад | `rage` | 0.66 |
| Net | `invisible` | 0.66 |
...
| Механика | Цена |
|---|---|
| Аура (HP/ATK/Armor) | 2.0 |
| Некромантия (`raise:1`) | 2.0 |
| Бусидо (`bushido`) | 2.0 |
| При смерти врага/союзника → хил базы + карта | 2.0 |
| При розыгрыше карты → хил базы | 2.0 |
| В начале хода → карта + доп.эссенция + хил всем на 1 | 2.0 |
| Активка "Жертва" (убить карту → эссенция + карта) | 2.0 |
| Активка "открыть карту" (untap) | 2.0 |
| Активка AOE по кнопке | 1.33 за каждую 1 ед. урона |
| Инкарнация (переменный X) | та же логика цены, что у рядовых, но **вдвое быстрее** за
  ту же цену (в 2 раза меньше X при равной цене) |

**Миры и Артефакты:** тело = **4.0** фикс (0/0 статов), любая способность = **+2.0** сверху.
Почти всегда итог **cost 6**.

**Спеллы:**

| Эффект | Цена |
|---|---|
| 1 карта добора | 1 эссенция |
| 1 урон точечно | 1 эссенция |
| Воскрешение на полном HP | 3 эссенции |
| Открыть карту (untap) | 2 эссенции (спеллом — та же цена, что и активкой выше) |
| +1 ATK до конца боя | 1.5 эссенции |

**Текущее состояние всех Уникальных карт** (для справки, statы уже посчитаны по таблице выше):

| Карта | Cost | HP/ATK | Теги |
|---|---|---|---|
| TEANTIST | 5 | 5/2 | draw:1, ward |
| ASLEX | 5 | 6/3 | aura:maxhp:1 |
| TUBORG | 5 | 5/2 | aura:atk:1, untamed, armor:1 |
| FAERON | 4 | 5/2 | burn, on_play_creature:1, untamed |
| NABUNAGI | 6 | 6/2 | bushido, armor:1, ward |
| REAPER | 5 | 6/3 | on_any_death_base:1 |
| RYVLEN | 4 | 5/3 | fear, draw_attack:1 |
| ABYSSWALKER | 5 | 5/2 | armor:1, aura:armor:1 |
| PHLEGMOR | 6 | 7/1 | raise:1, incarnation:2 |
| SEEKER | 4 | 5/3 | invisible, pierce |

**Миры/Артефакты (текущее состояние, все cost 6):** VALLEY (draw:1), DOMUS (world_maxhp:1),
THE BOOK (ess_add:1), FOUNTAIN (heal:1), HUNGER (on_own_death:1), NORRIA (world_armor:1),
SHARD (shard:2), ALTAR (sacrifice).

**Реализовано этой сессией, для справки:**
- `bolt:N` — точечный магический урон активкой на существе (не только у Umbasir — любая
  будущая карта может использовать этот тег), альтернатива AOE.
- ARCHIVE переделан на "+2 ATK до конца БОЯ" (переживает ход соперника, не только свой) —
  это закрывает старую идею "спелл с постоянным уроном/баффом" из бэклога, она реализована.
- Идея **"+X ATK за каждое сыгранное заклинание"** (`on_spell_cast:N`) — не реализована,
  перенесена в бэклог механик ниже (см. "Свободные механики" в общем списке идей).
...
## Next Up — куда фокус (записано 2026-07-13, вольный список, не строгий приоритет)

- Автор продолжит наполнять DEFS всеми путешественниками, навешивая статы/теги по
  Essence pricing shop (уже устоявшийся процесс — см. примеры сегодняшней сессии).
- **Состав стартовых колод обеих фракций (Classic) — решается ПОЗЖЕ**, когда определятся
  оставшиеся Миры/Артефакты/Уникальные — идея в том, что именно набор уников в стартере
  задаёт архетип, который новый игрок сможет прощупать в первой же партии (не механики
  вообще, а конкретный archetype-flavour через то, какие уники ему достались).
- Деккбилдинг-теги (gtype-cap и т.п.) — текущий набор устраивает, при необходимости
  добавятся новые, не горит.
- Ещё 2 World-трейта на путешественниках без назначенного бонуса — Optical Dope и
  Розовые облака (у второго уже есть кандидатура — `fear_immune`, не подтверждена).
- Автор будет постепенно добавлять спеллы — простые, переиспользуемые теги, специально
  чтобы разнообразить выбор для будущего стартера и коллекции, не обязательно новая
  механика на каждый спелл.
- Арт — у автора есть заготовки, будут добавляться по ходу.
- Звук — тоже требует внимания, детали не уточнены, держать в фокусе.
- **Что дальше по крупному:** лендинг, страница правил, лор, каталог — всё это ещё предстоит
  дополировать отдельными заходами.
...
Ничего из списка ниже НЕ реализовано и НЕ висит ни на одной карте (кроме armor/untamed — те уже в движке, см. Tag System и roadmap выше, но тоже пока без карты). Список — рабочая память для будущих сессий, а не готовые к реализации спеки.

Инкарнация X — существо само возрождается из своего кладбища через X ходов на полном HP. Отдельная механика от уже существующих raise/revive:full (те — чужой/один-раз триггер с другого существа; Инкарнация — свой отложенный self-revive по таймеру). Не решено: считать ходы чьи (свои ходы владельца или ходы вообще), что при уничтожении "в войд" (burn) — Инкарнация должна не работать, т.к. войд обычно означает "потеряно навсегда"?

Броня — синергия — свежедобавленный armor:N хорошо усилит 1/1 И подойдёт некоторым travelers через Mood/World трейт (конкретный трейт — решить позже, эстетичнее по смыслу).
...
taunt_break — снимает Provoke/Bushido с вражеского существа на 1 ход. Контрплей против танк-колод.

poison — вместо плоского burn:1/ход, каждое новое применение СТАКАЕТСЯ (+1 dmg/ход за стак) — растущая угроза, другой темп по сравнению с burn.
...
ess_steal:N (on_attack) — ворует N эссенции у соперника при атаке. Ресурсное давление; тематически ближе Jeet, у которого пока нет своего "экономического" архетипа (у Tea он уже есть через draw-engines — Teantist/Valley/Book, см. историю ребаланса выше).
...
vampiric (из ККИ Берсерк, «Вампиризм») — при уроне ОБЫЧНОЙ атакой (не магией) существо лечится на нанесённый урон (не больше, чем не хватает до maxHP). Ложится на ту же ветку doAttack(), что и rage/контратака — простой в реализации, взаимодействует с Ward ниже (Ward блокирует магию, но не обычную атаку — vampiric работает через обычную атаку, так что Ward его не остановит).

necrophage (из ККИ Берсерк, «Трупоедство») — при убийстве вражеского существа: изгнать его труп с кладбища противника (сразу в его void, минуя grave) + самому полностью вылечиться + снять с себя burning(аналог их "снять отравление"). Естественная контра нашей incarnation — убил Инкарнатора некрофагом → труп изгнан, воскреснуть уже не из чего.
...
on_play_gtype:N — при розыгрыше ЕЩЁ ОДНОЙ карты своего архетипа (gtype) наносит N урона сопернику. Естественно ложится на уже существующую систему архетипов (gtype:szg/ orb/drg/umb/mch/xui) и Squad — ещё один тип пейоффа архетипной колоды, отдельный от порога Squad. Пример: Dwarven Crossbowmen ("whenever owner summons a dwarf...").
...
shield (Divine Shield) — поглощает РОВНО ОДИН ближайший удар целиком, от ЛЮБОГО источника (физика ИЛИ магия), затем тег снимается. Третий слой защиты рядом с Бронёй (многоразовый пул, только физика) и Ward (безлимитный, только магия) — закрывает нишу, которую не закрывают они.
poisonous (Poisonous) — execute-механика, которой у нас вообще нет: ЛЮБОЙ урон от этого существа в бою убивает цель мгновенно, вне зависимости от цифр ATK/HP. Билд-эраунд потенциал ("маленький, но ядовитый убивает гиганта").
...
twinspell (Twinspell) — после розыгрыша в руку возвращается вторая (обычно более слабая) копия того же спелла, один раз.
frozen (Freeze) — отдельный дебафф от feared: Fear блокирует контратаку, Frozen — цель пропускает СВОЙ следующий ход атаки, но контратаковать в ответ всё ещё может. Разные ниши, оба имеют смысл рядом.
world_spelldmg:N (Spell Damage) — мир, усиливающий числа магического урона (Shard/ AOE/точечный спелл) у ВСЕХ карт владельца. Ложится на уже готовую world-инфраструктуру, по образцу world_armor/world_maxhp — не с нуля.
Не берём: Magnetic (позиционное "приклеивание" карт друг к другу — нет инфраструктуры), Discover (нужен отдельный UI "выбери 1 из 3 показанных карт" — тяжело, не тег), Adapt/Corrupt/Outcast (завязаны на позицию карты в руке/классовую специфику Hearthstone).
...
first_strike — см. отдельное обсуждение ниже (ломает симметрию одновременного обмена ударами, которая сейчас работает всегда).
trample (адаптированный под нас — у нас нет блокеров, зато есть базы) — если атака существа убивает цель, излишек урона сверх её HP уходит в базу противника.
...
hexproof — не может быть ВЫБРАН целью точечного спелла/Shard соперника вообще (в отличие от Ward — тот даёт спеллу отработать, но гасит урон; AOE Hexproof не спасает, т.к. AOE не таргетит конкретную карту).
...
Persist/Undying — обогащают уже собранную "семью воскрешений" (raise/revive/ incarnation/reborn/necrophage): воскресает, но с ПОСТОЯННЫМ модификатором статов при возврате (слабее либо сильнее) — гранулярность, которой пока не хватает.
flashback:N — спелл на кладбище можно разыграть ещё раз за N эссенции, один раз. Продолжает тему интереса к своему кладбищу (рядом с necrophage/grave_scale).
...
on_burn:N (скорее всего под 1/1) — при каждом сжигании ВЛАДЕЛЬЦЕМ карты (любой, не только этой) владелец добирает N карт. Вместе с unlimited_burn — потенциальный комбо-движок: сожги всю руку → пересобери её через добор, попутно получив кучу эссенции (сжигание уже даёт essMax+1/ess+1 за карту). Совсем незащищённый от абьюза комбо, если N=1 и сжигание безлимитно — сознательно нужно будет посмотреть на баланс отдельно, когда дойдём до реализации (не на брейншторм-этапе).
...
Переосмыслить pierce — идея от автора 2026-07-10: вместо текущего "игнорирует Provoke + можно бить базу напрямую" (см. Tag System — pierce, единственный потребитель — getTargetableCards()/canAttackBase() в game.js) сделать его "игнорирует Броню" (та же категория, что уже есть у AOE-активки/Shard/targeted-spell урона — dmgCard(...,true)). ⚠️ Открытый конфликт: pierce — core-тег Merchird (gtype:mch, "проникновение" по лору), и сейчас именно pierce даёt Merchird архетипу его identity — "продавливает" Provoke-стену. Если pierce полностью переопределить на "игнор брони", у Merchird пропадает механический ответ на Provoke — нужно либо решить, что это ок (Merchird становится анти-танк архетипом, не анти-Provoke), либо придумать Provoke-бypass отдельным новым тегом, чтобы не терять обе функции разом. Не решено, чисто идея на подумать.

Заклинание: дать +X Брони до конца ИГРЫ (не до конца хода) — новая идея, 2026-07-10. Нужен новый персистентный "перманентный" бонус-канал для брони, отдельно от armorMax'а, который не сбрасывается на endTurn()/applyAuras()-подобных пересчётах (broня и так уже многослойная — own+squad+aura+world, см. Squad System — добавление ещё и "постоянного спелл-бонуса" потребует аккуратно встроить его в recalcArmor(), как ещё один слагаемый newMax, не путая с temporary-механиками вроде tempAtkBonus).

ARCHIVE (spell_buff_temp:2, +2 ATK до конца ХОДА) → сделать бессрочным — автор: фактически то же самое, что "заклинание +X ATK до конца игры" уже почти реализовано, просто надо поменять ARCHIVE с temporary (tempAtkBonus, обнуляется в конце хода — см. "Targeted Spell System"/tempAtkBonus в Tag System) на постоянный эффект. Нужен новый персистентный ATK-канал (НЕ atkBonus — тот аура-only и полностью пересчитывается с нуля на каждом applyAuras(), постоянный спелл-бонус туда лезть не должен) — свежее поле вроде permAtkBonus, не трогаемое существующей аура-математикой. Хорошая пара с идеей про перманентную Броню выше — возможно, стоит решать оба сразу одним и тем же паттерном ("перманентный спелл-бонус" как отдельная категория, четвёртая рядом с own/squad/aura/world).

sacrifice_draw — ЧАСТИЧНО реализовано другим путём 2026-07-10: базовый пейофф Altar расширен до "И эссенция, И карта" (было только эссенция) — см. Version 1.01 roadmap выше. Изначальная идея была "карта ВМЕСТО эссенции" (альтернативный режим/отдельный тег) — то, что реализовано, это "карта В ДОПОЛНЕНИЕ" (просто усиление базового Altar). Если позже понадобится именно альтернативный режим (выбор между картой и эссенцией, а не оба сразу) — это всё ещё открытая для дизайна идея.
...
Невосприимчивость (working name: warded) — добавлено в список 2026-07-10, по прямому запросу автора, НЕ реализовано. Существо: (1) не получает урона от магии — та же категория "магического" урона, которую только что от armor отвязали в другую сторону (AOE-активка, Shard, targeted-spell урон, возможно и burn — уточнить у автора, входит ли burn сюда же, раз он уже трактуется как отдельная от armor категория), (2) не подвержено fear (тег не применяется), (3) не подвержено burn/поджогу (тег не применяется, "поджог" в формулировке автора). По сути — зеркальная пара к armor: armor блокирует физическое, warded блокирует магическое + два конкретных статуса. Открытые вопросы: считать ли Shard-урон "магией" тут же (Shard формально предмет, но урон уже трактуется как магический для armor-бypass — вероятно да, для консистентности); нужна ли отдельная реакция на попытку применить fear/burn к warded-существу (просто no-op, или отдельное лог-сообщение "X is warded — Fear has no effect"?).
