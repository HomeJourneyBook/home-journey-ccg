...
- [ ] **Кнопка "назад" во всех модалках до муллигана** — добавлено 2026-07-07 по запросу
  автора. Сейчас цепочка landing → deckPickerModal → vsAiPickerModal/deckBuilderModal →
  mulliganScreen идёт только вперёд, выйти можно только через закрытие/рестарт. Нужна кнопка
  назад на каждом шаге этой цепочки (включая сам муллиган — назад к выбору деки/фракции).
  Не уточнено: должна ли она сбрасывать уже собранную Rush-колоду при выходе из
  deckBuilderModal, или сохранять черновик.
- [ ] **Красивая модалка подбора карт** — добавлено 2026-07-07 по запросу автора. Судя по
  формулировке — про `deckBuilderModal` (Rush-деkбилдер, `js/deckbuilder.js`), но пересекается
  с уже существующим P3-пунктом "Апгрейд дизайна окна декбилдера" ниже (мелкие карты на
  лаптопе, нечитаемый степпер спеллов) — уточнить у автора, это тот же запрос или что-то
  отдельное (например сама модалка выбора Classic/Rush, `deckPickerModal`).
- [ ] **Прикрутить звуки от Муры** — добавлено 2026-07-07 по запросу автора. Возможно
  пересекается с существующим разделом "Sound checklist — ждём от друга автора" ниже
  (список звуков, которые готовит друг автора) — уточнить, тот же человек/тот же набор
  файлов или отдельная поставка.
...
- Снизу карты — выезжающая менюшка, поясняющая тип карты.|`classic` |Fixed 1st-edition starter — `buildDeck(f,'classic')` builds all 6 archetypes + all 5 legendaries + 3 copies of each spell (~45/46 Tea/Jeet). This is "every currently-implemented card" and is expected to keep changing size as balance testing continues — see `DECK_CONFIGS.classic` in deck.js. |
|`rush`    |No fixed list. The human player(s) build their own (min. `RUSH_MIN`=28 cards) via the deckbuilder screen (`js/deckbuilder.js`) — see below. The AI's Rush deck (VS AI only) is `buildAiRushDeck()`: a random RUSH_MIN-card sample of the same pool a human would pick from. |
