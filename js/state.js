...
function logTurnSnapshot(faction){
  ...
  G.logs.push({
    ...
    snapshot:{
      ...
      field:p.field.map(c=>`${c.name} (${c.hp}/${c.maxHp} HP, ${c.atk+(c.atkBonus||0)+rageAtkBonus(c)+(c.squadAtkBonus||0)+(c.tempAtkBonus||0)+(c.sagaAtkBonus||0)} ATK${c.armor?`, ${c.armor} Armor`:''})`),
...
function resetC(c){
  ...
  c._aloneSamuraiWasActive=undefined; // Stand Alone (2026-08-06) — тот же принцип: заново разыгранная карта не должна "помнить" прошлое активное состояние и молчать при следующем законном переходе
