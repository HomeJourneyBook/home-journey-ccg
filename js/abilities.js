  ...
  for(const a of abs){
    switch(a.effect){
      ...
      case 'random_spread':
        ...
        // 2026-07-24 — баг, автор поймал живьём). Пул целей — ЛЮБОЙ вражеский, включая
        // Ward: урон МОЖЕТ впустую уйти в Ward-цель (сгорит целиком, Ward блокирует магию
        // независимо от того, как цель была выбрана) — это ожидаемо и не баг.
        // ПЕРЕСМОТР (2026-07-30, по прямому запросу автора) — невидимых/нераскрытый stealth
        // теперь ИСКЛЮЧАЕМ из пула (см. isSpellTargetable() ниже) — раньше здесь стояло
        // обратное решение ("честная случайность, может улететь и в невидимого"), но автор
        // передумал: случайный выбор ОДНОЙ жертвы — это по сути тот же targeting, что и у
        // обычного спелла, просто рандом вместо игрока решает кого; Invisible должен от
        // этого защищать так же, как от прицельного спелла.
        // ДОБАВЛЕНО (2026-07-24, по прямому запросу автора, второй заход) — вражеская
        // БАЗА теперь тоже в пуле случайных целей, наравне с существами. В отличие от
        // существа, база НИКОГДА не выбывает из пула (нет "смерти" в рамках одного каста,
        // просто копит очки урона).
        // РЕДИЗАЙН РАСПРЕДЕЛЕНИЯ (2026-07-24, по прямому запросу автора, третий заход) —
        // автор поймал живьём: при 2 существах на поле (пул из 3 целей — 2 существа+база)
        // и 3 очках урона весь урон мог случайно улететь целиком в одну цель (например,
        // всё в базу) чисто по теории вероятности чистого рандома. Это ощущалось как баг,
        // не как "честная случайность". Новое правило в 2 фазы:
        //   Фаза 1 — ГАРАНТИРОВАННЫЙ ОХВАТ: пока в пуле есть ещё НЕ тронутая цель И ещё
        //   остались очки урона — следующее очко идёт в СЛУЧАЙНУЮ ещё-нетронутую цель
        //   (шаффл пула, проход по одному разу на каждую). Это гарантирует то, что описал
        //   автор: 3 цели/3 очка → каждая цель получает ровно по 1; 2 цели/3 очка → обе
        //   получают по 1, третье очко решается фазой 2; 1 цель/N очков → фаза 1 сразу
        //   заканчивается после первого очка (больше нечего "охватывать"), всё остальное
        //   уходит в фазу 2 на ту же единственную цель.
        //   Фаза 2 — ДОБИВАНИЕ (только если очков больше, чем целей в пуле) — та же чисто
        ...
        {
          // Невидимость/нераскрытый stealth (2026-07-30, ПЕРЕСМОТР по прямому запросу
          // автора — раньше тут нарочно было "любой враг, включая невидимых, честный
          // рандом", см. историю в комментарии выше о фазах). Автор передумал: у Invisible
          // правило "Cannot be TARGETED" — а случайный выбор ОДНОЙ жертвы из пула это то же
          // самое "targeting", просто с рандомом вместо игрока/ИИ в роли решающего. В
          // отличие от настоящего blanket-AOE (бьёт вообще всех без разбора, никого не
          // "выбирая") — Shrapnel/Nana выбирают ОДНУ конкретную цель за раз, значит должны
          // уважать ту же targetability, что и обычный таргетируемый спелл. Использует тот
          // же isSpellTargetable(), что и TARGETED_SPELL_TAGS (game.js) — база в фильтр не
          // попадает (это не card-объект), её targetability не проверяется.
          const pool=[...G[oppK].field].filter(t=>isSpellTargetable(t,G[oppK].field));
          ...
          for(let i=0;i<shuffled.length && pointsLeft>0;i++){
            ...
            pointsLeft--;
            if(entry.hpLeft<=0){
              const idx=alive.indexOf(entry);
              if(idx>=0) alive.splice(idx,1);
            }
          }

          // Фаза 2 — оставшиеся очки (если пул закончился раньше, чем очки урона).
