...
const SAGA_TEA_DECK = [
  // cost1 (3 существа + 5 спеллов = 8) — 2026-08-08, по прямому запросу автора: все три
  // ваниль-путешественника заменены на архетипных близнецов, которые раньше нигде не
  // использовались (ни в Classic, ни здесь) — t_trvl18_w(Mechird)→t_trvl548_w,
  // t_trvl91_w(Dreegan)→t_trvl201_w, t_trvl52_w(Umbasir)→t_trvl21_w.
  't_trvl548_w','t_trvl201_w','t_trvl21_w',
  ...
  // ── 2-й заход того же дня, по прямому запросу автора: все пять ваниль cost2 заменены на
  // архетипных близнецов, свободных нигде (тот же принцип, что у cost1 выше):
  // t_trvl28_w(Szarg)→t_trvl33_w, t_trvl870_w(Szarg)→t_trvl890_w, t_trvl868_w(Orbiton)→
  // t_trvl596_w(NAMIRA), t_trvl218_w(Orbiton)→t_trvl465_w, t_trvl39_w(Xuiqtr)→t_trvl32_w.
  't_trvl33_w','t_trvl890_w','t_trvl596_w','t_trvl465_w','t_trvl32_w',
  't_sp15','t_sp13','t_sp26',
  // cost3 (7 существ, включая 2 Saga + 3 спелла = 10) — 2026-08-08, по прямому запросу
  // автора (3-й заход того же дня): t_trvl26_w(Xuiqtr)→t_trvl951_w(Xuiqtr, тоже cost3,
  // раньше нигде не использовалась), t_trvl34_w(Szarg)→t_trvl694_w(Szarg, тоже cost3,
  // свободна), t_trvl6_w(Umbasir)→t_trvl279_w(Umbasir, тоже cost3, свободна).
  't_trvl102_w','t_trvl100_w','t_trvl694_w','t_trvl420_w','t_trvl1_w','t_trvl279_w','t_trvl951_w',
  ...
  // 2026-08-08, по прямому запросу автора (3-й заход того же дня): t_trvl495_w(Mechird)→
  // t_trvl699_w(Mechird, тоже cost4, свободна), t_trvl583_w(Umbasir)→t_trvl372_w(Umbasir,
  // тоже cost4, свободна).
  't_trvl372_w','t_trvl699_w','t_trvl847_w','t_sp19','t_a2',
...
const FOXY_JEET_DECK = [
  // cost1 (3 существа + 5 спеллов = 8) — 2026-08-08, по прямому запросу автора: j_trvl724_w
  // (Mechird, cost1) заменён на j_trvl571_w — тоже cost1 Mechird, но раньше нигде не
  // использовался ни в Classic, ни в этой колоде (не создаёт повтора карты между колодами).
  'j_trvl571_w','j_trvl347_w','j_trvl54_w',
  ...
  // ── 2-й заход того же дня, по прямому запросу автора, три замены разом, все — "другая
  // карта того же архетипа, которая раньше нигде не использовалась" (ни в Classic, ни здесь):
  // j_trvl359_w (Orbiton) → j_trvl967_w (ORYTH, тоже cost2 Orbiton, свободна); j_trvl50_w
  // (Xuiqtr) → j_trvl0_w (тоже cost2 Xuiqtr, свободна); j_trvl934_w (Umbasir) → j_trvl687_w
  // — j_trvl934_w был ЕДИНСТВЕННЫМ cost2 Umbasir в игре (нет свободного аналога того же
  // cost), поэтому вместо перестановки автор попросил завести НОВУЮ карту j_trvl687_w
  // (см. её карточку в data.js, статы по формуле того же архетипа) специально под этот слот.
  'j_trvl7_w','j_trvl687_w','j_trvl457_w','j_trvl967_w','j_trvl0_w',
  'j_sp15','j_sp1','j_sp26',
  ...
  // ── 2-й заход того же дня, по прямому запросу автора, ещё 2 замены, тот же принцип "другая
  // карта архетипа, раньше нигде не использованная": j_trvl36_w (Dreegan) → j_trvl757_w
  // (тоже cost3 Dreegan, свободна — заодно освобождает j_trvl36_w от двойного использования
  // в Classic И здесь одновременно); j_trvl76_w (Xuiqtr) → j_trvl3_w (тоже cost3 Xuiqtr,
  // свободна, тот же повод).
  't_trvl13_w','t_trvl51_w','j_trvl25_w','j_trvl454_w','j_trvl757_w','j_trvl3_w',
  'j_sp2','j_sp20','j_sp16','j_a2',
  // cost4 (4 существа, включая новый Xuiqtr #806 (см. её комментарий ниже) + Umbasir #20 = 5.
  // 2026-08-08, по прямому запросу автора (2-й заход того же дня): j_trvl196_w (cost4 Szarg,
  // pierce+market+incarnation:4) убран целиком, заменён на j_trvl20_w (cost4 Umbasir,
  // bolt:1+vanguard+untamed). Предыдущий заход (комментарий ниже) заменил j_trvl133_w на
  // j_trvl196_w — эта замена теперь тоже снята, j_trvl196_w больше в колоде нет.
  ...
  't_trvl24_w','j_trvl320_w','j_trvl806_w','j_trvl20_w','j_sp25',
  // cost5 (3 существа, все три Foxy + 2 спелла = 5) — 2026-08-08 (2-й заход того же дня, по
  // прямому запросу автора): j_trvl1032_w (cost5 Umbasir, bolt:1+frost+death_armor:2) убран
  // целиком, заменён на j_trvl248_w (cost5 Umbasir, bolt:1+shield+ward). Комментарий ниже —
  // от ПЕРВОГО захода того же дня, оставлен для контекста прежней структуры слота:
  ...
  'j_trvl15_w','j_trvl48_w','j_trvl248_w','j_sp9','j_sp6',
