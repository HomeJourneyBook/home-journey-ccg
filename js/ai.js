...
const AI_WEIGHTS = {
  ...
  tagBonus: { // безусловный бонус тега, независимо от ситуации на поле
    ...
    alone_samurai:0.3, // LAST KIIRO (2026-08-06, добавлена в тот же день, что и сама карта) — условный
                      // +2 ATK, ПОКА карта одна на поле (см. applyAuras()/game.js). Вес на уровне
                      // atk_vs_burning/atk_vs_feared(0.3) — тоже условный бонус, а не безусловный тег:
    ...
    founder_of_saga:0.6, // KREATIV (2026-08-06) — сильная безусловная аура: любая Saga-карта
                      // на поле (своя и новая) мгновенно и НАВСЕГДА доводится до максимума
                      // (см. _forceSagaMax()/game.js). Вес на уровне shield/necrophage(0.6) —
                      // не завязан на условие поля, эффект сразу же и полностью реализуется.
    avenge_foxy_miss:0.35, // THUG ASTEANAUT (2026-08-06) — реактивный punish-эффект: сила
                      // напрямую зависит от того, сколько Foxy-карт СВОИХ на поле и как
                      // часто их реально атакуют (50% сам по себе шанс промаха) — в отличие
                      // от founder_of_saga выше это НЕ безусловная гарантия, вес ниже, ближе
                      // к thorns/shadow_shield(0.3), чуть выше — за счёт того, что 1 урон
                      // существу может добить его, а не просто чипнуть базу.
    synergy_saga_count:0.5, // BAN'KAI (2026-08-06) — count-scaling payoff (+1 maxHP/+1 ATK
                      // за КАЖДУЮ saga-карту на поле, потолок 3) — в голой classic-колоде
                      // (без build-around плотности тега) часто просто ваниль 6/2 за 5, но
                      // при нескольких saga-картах разом даёт до +3/+3 одновременно — вес на
                      // уровне incarnation/saga самого тега (0.5), не выше: реализация
                      // ценности зависит от состава колоды/борда, не гарантирована.
    synergy_foxy_count:0.5, // COPE GUARDIAN (2026-08-06) — зеркало BAN'KAI, тот же вес, та же логика.
