...
function aiResolvePendingSpellTarget(){
  ...
  if(G.phase==='spellDmgTarget'){
    const targets=G[humanF].field.filter(c=>!c.spell&&!c.world&&!c.artifact&&!hasTag(c,'ward')&&!c.frozen&&!(hasTag(c,'shield')&&!c.shieldConsumed)&&isSpellTargetable(c,G[humanF].field));
  ...
  if(G.phase==='spellExecuteHalfTarget'){
    ...
    const legalTargets=G[humanF].field.filter(c=>!c.spell&&!c.world&&!c.artifact&&!hasTag(c,'ward')&&!c.frozen&&!(hasTag(c,'shield')&&!c.shieldConsumed)&&isSpellTargetable(c,G[humanF].field));
...
function aiTryUseShard(forceNow){
  ...
  const enemyField=G[humanF].field.filter(c=>!c.spell&&!c.world&&!c.artifact&&!hasTag(c,'ward')&&!c.frozen&&!(hasTag(c,'shield')&&!c.shieldConsumed)&&isSpellTargetable(c,G[humanF].field));
  if(enemyField.length===0) return false;
...
function aiTryUseBolt(){
  ...
  // Видимость (2026-07-18): invisible/нераскрытый stealth тоже нельзя выбрать целью.
  const enemyField=oppField.filter(c=>!c.spell&&!c.world&&!c.artifact&&!hasTag(c,'ward')&&!c.frozen&&!(hasTag(c,'shield')&&!c.shieldConsumed)&&isSpellTargetable(c,G[humanF].field));
  if(enemyField.length===0) return false;
...
function aiSpellHasValidTarget(card){
  ...
  if(hasTag(card,'spell_dmg_target')){
    return G[humanF].field.some(c=>!c.spell&&!c.world&&!c.artifact&&!hasTag(c,'ward')&&!c.frozen&&!(hasTag(c,'shield')&&!c.shieldConsumed)&&isSpellTargetable(c,G[humanF].field));
  }
